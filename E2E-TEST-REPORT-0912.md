...
- **#33（已修复，`73c9441`+`dd4b665`）**：源 PVC 在 **Reserved 阶段**被删除时，reconcile 循环报 "reserve volume: read source PVC"（预留重校验）而非转入 Failed。修复：四个执行器新增 `FailSourceDeleted`——controller 在驱动执行前探测计划内源 PVC，缺失即经 `m.fail` 记录失败原因并转入 Failed 终态（幂等：Failed/Completed/Aborted/RolledBack 不重复记录；删除收敛路径不受影响）。**集群复现验证**：Reserved 阶段删源 PVC/Pod 后，工作流自主转 Failed、CR 干净删除、reconciler 仅 1 条错误（无循环）。
