...
- Session commands print phase-aware next steps, verification commands, and validated dry-run/execute pairs on stderr. Suggested `pvc-migrate` and `kubectl` commands preserve the active kubeconfig, context, session namespace, and explicitly changed execution settings they need. JSON and YAML results remain a single structured document on stdout. With `--log-format=json`, stderr is JSON Lines for progress events, guidance, and failures.
