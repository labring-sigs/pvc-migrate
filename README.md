...
- For an active OpenEBS LVM LocalPV whose StorageClass does not declare `shared: "yes"`, a warm-copy plan offers `--precopy-passes 0` for an offline final sync. `--openebs-lvm-enable-shared` is an explicit alternative: it patches the matching existing `LVMVolume.spec.shared` to `yes`, then verifies a same-node second-Pod read-write mount without writing data. This changes the retained OpenEBS CR and does not provide cross-node RWX storage.
