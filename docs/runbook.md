...
Review the planned controller kind and served KubeBlocks API version. For InstanceSet-backed components, a selected leader, primary, or master needs a supported `--kubeblocks-candidate` strategy or `--allow-leader-downtime`. A candidate uses the served Switchover action, or the validated native replica-set switchover for MongoDB. The validated Redis addon has no Switchover action; omit `--kubeblocks-candidate` and use `--allow-leader-downtime` for a Redis primary. InstanceSet pause sets `spec.paused=true`, deletes only the selected Pod with a UID precondition, and restores the original pause representation after activation. Legacy StatefulSet-backed components reject `--kubeblocks-candidate`; `--allow-leader-downtime` has no effect because their Stop/Start OpsRequest pauses every instance in the affected Cluster or component. The apps API payload uses `clusterRef` and pauses the Cluster; the operations API payload uses `clusterName` and can target the selected component. The adapter waits for consumers to disappear before final sync. The `kubeblocks.io/reconcile` annotation triggers reconciliation and does not pause it.
