...
Quota evaluation uses Kubernetes `resource.Quantity` in every namespace where a workflow creates resources, including separate source, staging, destination, and session namespaces. It reads admitted `ResourceQuota.status.hard` and `status.used`, applies Pod and VolumeAttributesClass scopes, models LimitRange defaults, and ignores zero demand. Unscoped Pod quota uses the workflow's maximum concurrent Pod count; `Terminating` and `NotTerminating` quota use their separate phase peaks for probes, reservations, and transfer tools. PVC replacement projections use the larger of requested and allocated storage so recovery from a failed expansion matches the Kubernetes quota evaluator. Capacity awareness aggregates requested PVC capacity by StorageClass, matches namespaced `CSIStorageCapacity` objects against target-node labels, and checks both available capacity and the largest requested volume. Actual destination binding remains the authoritative storage-capacity test.

Tool containers set CPU and memory requests/limits to `0` and ephemeral-storage requests to `0`. The ephemeral-storage limit remains unset so kubelet can account for tool logs and writable-layer usage without immediately evicting a container whose limit is `0`. Kubernetes does not require or charge an omitted ephemeral-storage limit to `limits.ephemeral-storage`. A namespace LimitRange `default` can make that limit explicit, and the planner then includes it. A `defaultRequest` only affects an omitted request and does not create a limit. Object-count quota for Pods, Jobs, Deployments, ReplicaSets, Secrets, Services, ServiceAccounts, ConfigMaps, PVCs, and Leases still applies and remains part of the plan estimate.
