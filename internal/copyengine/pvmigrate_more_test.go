...
func TestAllUpstreamStrategiesMapExactly(t *testing.T) {
	...
	for input, want := range wants {
		got, err := StrategyValueForTest(input)
		if err != nil {
	...
	for _, input := range []string{"", "MOUNT", "cluster-ip", "exec", " mount"} {
		_, err := StrategyValueForTest(input)
	...
	for _, strategy := range pvmigrate.AllStrategies {
		got, err := StrategyValueForTest(string(strategy))
		if err != nil || got != strategy {
			t.Fatalf(
				"upstream strategy %q is not supported by the adapter: value=%q error=%v",
				strategy,
				got,
				err,
			)
...
func TestCopyBuildsCompleteUpstreamMigrationContract(t *testing.T) {
	...
	engine := NewPVMigrateForTest(func(_ context.Context, migration pvmigrate.Migration) error {
		captured = migration
		return nil
	})
	writer := &bytes.Buffer{}
	...
	if err := engine.Copy(
		context.Background(),
		request,
		func(progress Progress) { updates = append(updates, progress) },
	); err != nil {
		t.Fatal(err)
	}

	if captured.ID != OperationID(request) ||
		captured.Source != (pvmigrate.PVC{KubeconfigPath: "/tmp/kubeconfig", Context: "cluster-context", Namespace: "source-ns", Name: "source-pvc", Path: "data/mysql/."}) ||
		captured.Dest != (pvmigrate.PVC{KubeconfigPath: "/tmp/kubeconfig", Context: "cluster-context", Namespace: "target-ns", Name: "target-pvc", Path: "restored/mysql/."}) {
		t.Fatalf("upstream PVC request=%#v", captured)
	}

	if !captured.DeleteExtraneousFiles || captured.IgnoreMounted ||
		!captured.SourceMountReadWrite ||
		!captured.IgnoreSizes ||
		!captured.NoCompress ||
		!captured.StructuredLogs ||
	...
	if captured.RsyncExtraArgs != "-HAXS --numeric-ids --checksum --mkpath" {
		t.Fatalf("upstream rsync args=%q", captured.RsyncExtraArgs)
	}

	if captured.HelmTimeout != request.HelmTimeout ||
		len(captured.Strategies) != len(request.Strategies) ||
		captured.Strategies[0] != pvmigrate.Mount ||
		captured.Strategies[4] != pvmigrate.Local {
		t.Fatalf(
			"upstream execution fields timeout=%s strategies=%v",
			captured.HelmTimeout,
			captured.Strategies,
		)
	}
	...
	if len(captured.HelmValues) != len(kube.ToolSecurityContextHelmValues())+1 ||
		...
		t.Fatalf(
			"upstream Helm values=%v stringValues=%v",
			captured.HelmValues,
			captured.HelmStringValues,
		)
	}

	if len(updates) != 2 || updates[0].State != "running" || updates[1].State != "completed" ||
		updates[0].Mode != ModeFinal ||
		updates[1].Attempt != request.Attempt {
		t.Fatalf("progress=%#v", updates)
	}
}

func TestCopyUsesDestinationKubeconfigCurrentContextWhenContextIsEmpty(t *testing.T) {
	var captured pvmigrate.Migration
	...
	request := Request{
		SessionID:                 "cross-cluster",
		ToolImage:                 "registry.example/pvc-migrate:tool",
		Source:                    domain.ObjectReference{Namespace: "source", Name: "data"},
		Destination:               domain.ObjectReference{Namespace: "destination", Name: "data"},
		KubeconfigPath:            "/tmp/source.yaml",
		Context:                   "source-context",
		DestinationKubeconfigPath: "/tmp/destination.yaml",
	...
	if captured.Dest.KubeconfigPath != request.DestinationKubeconfigPath ||
		captured.Dest.Context != "" {
		t.Fatalf("destination connection=%#v", captured.Dest)
	}
}
...
func TestTransferEnginePathAlwaysCopiesDirectoryContents(t *testing.T) {
	for _, test := range []struct {
		...
		if got := TransferEnginePathForTest(test.input); got != test.want {
...
func TestCopyBuildsWarmUpstreamMigrationContract(t *testing.T) {
	...
	if !captured.IgnoreMounted || captured.RsyncExtraArgs != "-HAXS --numeric-ids" ||
		captured.HelmTimeout != 10*time.Minute {
		t.Fatalf(
			"warm upstream migration: ignoreMounted=%t rsyncArgs=%q helmTimeout=%s",
			captured.IgnoreMounted,
			captured.RsyncExtraArgs,
			captured.HelmTimeout,
		)
	}
}
...
func TestCopyReportsUpstreamFailureWithStableOperationIdentity(t *testing.T) {
	cases := []struct {
		...
		{
			name:     "final checksum and explicit timeout",
			mode:     ModeFinal,
			verify:   true,
			timeout:  time.Second,
			strategy: "clusterip",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if domain.CategoryOf(err) != domain.ErrorCopy {
				t.Fatalf(
					"Copy() error=%v category=%q logs=%s",
					err,
			...
			if len(updates) != 2 || updates[0].State != "running" ||
				updates[0].Message != operation ||
...
func TestCopyClassifiesDestinationENOSPCFromStructuredFailureTail(t *testing.T) {
	...
	engine := NewPVMigrateForTest(func(ctx context.Context, migration pvmigrate.Migration) error {
		runContextDone = ctx.Done()

		migration.Logger.Warn(
			"data mover failed",
			"tail",
	...
	case <-runContextDone:
	default:
		t.Fatal("upstream strategy context was not canceled after ENOSPC")
	}

	if strings.Contains(logs.String(), "will try with the remaining strategies") ||
		!strings.Contains(
			logs.String(),
...
func TestProgressSerializesStableFields(t *testing.T) {
	...
	if progress.Mode != ModeFinal || progress.Attempt != 3 || progress.State != "completed" ||
		progress.Message != "pm-id" ||
		progress.Bytes != 42 {
...
func TestClassifyRunErrorPreservesTimeoutAndCancellation(t *testing.T) {
	...
	tests = append(tests,
		...
		testCase{name: "copy error", err: io.EOF, category: domain.ErrorCopy},
	)

	tests = append(tests, testCase{
		name: "deadline from context", err: io.EOF, category: domain.ErrorTimeout, expired: true,
	})
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := ClassifyRunErrorForTest(ctx, "pm-test", test.err, false)
