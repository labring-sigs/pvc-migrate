...
func (p Printer) printClusterPodMigration(object *v1alpha1.ClusterPodMigration) error {
	if object == nil {
		return nil
	}
	...
	checkpoints := make(
		map[string]v1alpha1.ClusterPodMigrationVolumeStatus,
	...
	if plan := object.Status.Plan; plan != nil {
		for _, volume := range plan.Volumes {
			...
			if err := printPodMigrationVolume(
				w,
				string(plan.SourceNamespace)+"/"+volume.SourcePVC.Name,
				checkpoint.Sync.WarmCompletedAt != nil,
				checkpoint.Sync.FinalCompletedAt != nil,
				checkpoint.Activation.ActivatedAt != nil,
				checkpoint.Activation.RolledBackAt != nil,
