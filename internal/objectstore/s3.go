...
func New(ctx context.Context, cfg Config) (*Store, error) {
	...
	if cfg.AccessKey != "" {
		loadOptions = append(loadOptions, awsconfig.WithCredentialsProvider(credentials.NewStaticCredentialsProvider(cfg.AccessKey, cfg.SecretKey, cfg.SessionToken)))
	}
	awsCfg, err := awsconfig.LoadDefaultConfig(ctx, loadOptions...)
	if err != nil {
		return nil, domain.WrapError(domain.ErrorValidation, "S3 configuration", "load AWS configuration", err)
	}
	resolved, err := awsCfg.Credentials.Retrieve(ctx)
	if err != nil {
		return nil, domain.WrapError(domain.ErrorPrecondition, "S3 credentials", "resolve credentials", err)
	}
	...
	return &Store{
		client:      client,
		config:      cfg,
		credentials: Credentials{AccessKey: resolved.AccessKeyID, SecretKey: resolved.SecretAccessKey, SessionToken: resolved.SessionToken},
	}, nil
}
...
func ValidateConfig(cfg Config) error {
	if !safeSegment.MatchString(cfg.Bucket) {
		return domain.NewError(domain.ErrorValidation, "S3 configuration", "bucket must contain only alphanumeric characters, dots, underscores, and hyphens")
	}
	if !safeSegment.MatchString(cfg.Name) {
		return domain.NewError(domain.ErrorValidation, "S3 configuration", "name must contain only alphanumeric characters, dots, underscores, and hyphens")
	}
	if cfg.Prefix != "" {
		for _, segment := range strings.Split(cfg.Prefix, "/") {
			if !safeSegment.MatchString(segment) {
				return domain.NewError(domain.ErrorValidation, "S3 configuration", "prefix must contain slash-separated safe path segments")
			}
		}
	}
	if (cfg.AccessKey == "") != (cfg.SecretKey == "") {
		return domain.NewError(domain.ErrorValidation, "S3 credentials", "access key and secret key must be supplied together")
	}
	for name, value := range map[string]string{
		...
		if strings.ContainsAny(value, "\r\n\x00") || strings.TrimSpace(value) != value {
			return domain.NewError(domain.ErrorValidation, "S3 configuration", fmt.Sprintf("%s contains unsafe whitespace", name))
		}
	}
	if cfg.Endpoint != "" {
		parsed, err := url.Parse(cfg.Endpoint)
		if err != nil || parsed.Host == "" || (parsed.Scheme != "https" && parsed.Scheme != "http") {
			return domain.NewError(domain.ErrorValidation, "S3 configuration", "endpoint must be an absolute HTTP or HTTPS URL")
		}
		if parsed.Scheme == "http" && !cfg.AllowInsecureEndpoint {
			return domain.NewError(domain.ErrorPrecondition, "S3 configuration", "HTTP endpoint requires --allow-insecure-endpoint")
		}
	}
	if cfg.SSEKMSKeyID != "" && cfg.ServerSideEncryption != "aws:kms" {
		return domain.NewError(domain.ErrorValidation, "S3 encryption", "SSE KMS key ID requires --s3-server-side-encryption=aws:kms")
	}
	switch cfg.ServerSideEncryption {
	case "", "AES256", "aws:kms":
	default:
		return domain.NewError(domain.ErrorValidation, "S3 encryption", "server-side encryption must be AES256 or aws:kms")
	}
	return nil
}
...
func (s *Store) Manifest(ctx context.Context) (*Manifest, error) {
	...
	if err != nil {
		...
		return nil, wrapS3Error(ctx, domain.ErrorPrecondition, "S3 manifest", "read completion manifest", err)
	}
	defer output.Body.Close()
	data, err := io.ReadAll(io.LimitReader(output.Body, 1<<20))
	if err != nil {
		return nil, wrapS3Error(ctx, domain.ErrorPrecondition, "S3 manifest", "read completion manifest body", err)
	}
	var manifest Manifest
	if err := json.Unmarshal(data, &manifest); err != nil {
		return nil, domain.WrapError(domain.ErrorPrecondition, "S3 manifest", "decode completion manifest", err)
	}
	if manifest.Version != manifestVersion || manifest.Bucket != s.config.Bucket || manifest.Prefix != s.config.Prefix || manifest.Name != s.config.Name {
		return nil, domain.NewError(domain.ErrorConflict, "S3 manifest", "completion manifest identity is invalid")
	}
...
func (s *Store) PutManifest(ctx context.Context, manifest Manifest) error {
	...
	if err != nil {
		return domain.WrapError(domain.ErrorInternal, "S3 manifest", "encode completion manifest", err)
	}
	...
	if _, err := s.client.PutObject(ctx, input); err != nil {
		return wrapS3Error(ctx, domain.ErrorConflict, "S3 manifest", "publish immutable completion manifest", err)
	}
	return nil
}
...
func (s *Store) Inventory(ctx context.Context) (Inventory, error) {
	...
	for {
		...
		if err != nil {
			return Inventory{}, wrapS3Error(ctx, domain.ErrorPrecondition, "S3 inventory", "list backup objects", err)
		}
		for _, object := range output.Contents {
			...
			if etag == "" {
				return Inventory{}, domain.NewError(domain.ErrorPrecondition, "S3 inventory", fmt.Sprintf("object %s has no ETag", key))
			}
			size := aws.ToInt64(object.Size)
			if size < 0 {
				return Inventory{}, domain.NewError(domain.ErrorPrecondition, "S3 inventory", fmt.Sprintf("object %s has a negative size", key))
			}
			entries = append(entries, inventoryEntry{Key: strings.TrimPrefix(key, prefix), Size: size, ETag: etag})
		}
		if !aws.ToBool(output.IsTruncated) {
			break
		}
		if output.NextContinuationToken == nil || aws.ToString(output.NextContinuationToken) == "" {
			return Inventory{}, domain.NewError(domain.ErrorPrecondition, "S3 inventory", "truncated object listing has no continuation token")
		}
	...
	for _, entry := range entries {
		if err := encoder.Encode(entry); err != nil {
			return Inventory{}, domain.WrapError(domain.ErrorInternal, "S3 inventory", "encode inventory entry", err)
		}
		totalBytes += entry.Size
		if totalBytes < 0 {
			return Inventory{}, domain.NewError(domain.ErrorPrecondition, "S3 inventory", "backup object size total overflowed")
		}
	}
	return Inventory{ObjectCount: int64(len(entries)), TotalBytes: totalBytes, SHA256: hex.EncodeToString(hasher.Sum(nil))}, nil
}
...
func (s *Store) VerifyInventory(ctx context.Context, manifest Manifest) error {
	...
	if current.ObjectCount != manifest.ObjectCount || current.TotalBytes != manifest.TotalBytes || current.SHA256 != manifest.InventorySHA256 {
		return domain.NewError(domain.ErrorConflict, "S3 inventory", fmt.Sprintf("published backup objects changed: count=%d/%d bytes=%d/%d digest=%s/%s", current.ObjectCount, manifest.ObjectCount, current.TotalBytes, manifest.TotalBytes, current.SHA256, manifest.InventorySHA256))
	}
	return nil
}
...
func (s *Store) AcquireLock(ctx context.Context, holder string, ttl time.Duration) (string, error) {
	if holder == "" || ttl <= 0 {
		return "", domain.NewError(domain.ErrorValidation, "S3 lock", "holder and positive TTL are required")
	}
	...
	if err == nil {
		...
		if etag == "" {
			return "", domain.NewError(domain.ErrorConflict, "S3 lock", "S3 did not return an ETag for the newly acquired lock")
		}
		return etag, nil
	}
	...
	output, err = s.client.PutObject(ctx, input)
	if err != nil {
		return "", wrapS3Error(ctx, domain.ErrorConflict, "S3 lock", "replace expired backup lock", err)
	}
	newETag := aws.ToString(output.ETag)
	if newETag == "" {
		return "", domain.NewError(domain.ErrorConflict, "S3 lock", "S3 did not return an ETag for the replaced lock")
	}
	return newETag, nil
}
...
func (s *Store) ReleaseLock(ctx context.Context, etag string) error {
	if etag == "" {
		return domain.NewError(domain.ErrorConflict, "S3 lock", "lock ETag is required for conditional release")
	}
	input := &s3.DeleteObjectInput{Bucket: aws.String(s.config.Bucket), Key: aws.String(s.lockKey())}
	input.IfMatch = aws.String(etag)
...
func (s *Store) RenewLock(ctx context.Context, holder, etag string, ttl time.Duration) (string, error) {
	if holder == "" || etag == "" || ttl <= 0 {
		return "", domain.NewError(domain.ErrorValidation, "S3 lock", "holder, ETag, and positive TTL are required")
	}
	current, currentETag, err := s.readLock(ctx)
	if err != nil {
		return "", wrapS3Error(ctx, domain.ErrorConflict, "S3 lock", "read lock before renewal", err)
	}
	if current == nil || current.Holder != holder || currentETag != etag {
		return "", domain.NewError(domain.ErrorConflict, "S3 lock", "lock ownership changed before renewal")
	}
	data, _ := json.Marshal(Lock{Holder: holder, ExpiresAt: time.Now().UTC().Add(ttl)})
	input := &s3.PutObjectInput{
		Bucket:      aws.String(s.config.Bucket),
		Key:         aws.String(s.lockKey()),
		Body:        bytes.NewReader(data),
	...
	newETag := aws.ToString(output.ETag)
	if newETag == "" {
		return "", domain.NewError(domain.ErrorConflict, "S3 lock", "S3 did not return an ETag for the renewed lock")
	}
	return newETag, nil
}
...
func validateManifest(manifest Manifest) error {
	if manifest.Version != manifestVersion {
		return domain.NewError(domain.ErrorPrecondition, "S3 manifest", "unsupported completion manifest version")
	}
	if manifest.CreatedAt.IsZero() {
		return domain.NewError(domain.ErrorPrecondition, "S3 manifest", "completion manifest createdAt is required")
	}
	if manifest.SourceNamespace == "" || manifest.SourcePVC == "" || manifest.SourcePVCUID == "" {
		return domain.NewError(domain.ErrorPrecondition, "S3 manifest", "completion manifest source identity is incomplete")
	}
	if manifest.Capacity == "" {
		return domain.NewError(domain.ErrorPrecondition, "S3 manifest", "completion manifest capacity is required")
	}
	capacity, err := resource.ParseQuantity(manifest.Capacity)
	if err != nil {
		return domain.WrapError(domain.ErrorPrecondition, "S3 manifest", "parse completion manifest capacity", err)
	}
	if capacity.Sign() <= 0 {
		return domain.NewError(domain.ErrorPrecondition, "S3 manifest", "completion manifest capacity must be positive")
	}
	if manifest.VolumeMode == "" || manifest.Consistency == "" || manifest.Compression == "" {
		return domain.NewError(domain.ErrorPrecondition, "S3 manifest", "completion manifest storage and consistency fields are required")
	}
	if manifest.VolumeMode != "Filesystem" {
		return domain.NewError(domain.ErrorPrecondition, "S3 manifest", "S3 file backup manifest must use Filesystem VolumeMode")
	}
	if manifest.ObjectCount < 0 || manifest.TotalBytes < 0 {
		return domain.NewError(domain.ErrorPrecondition, "S3 manifest", "completion manifest inventory counts must be non-negative")
	}
	if len(manifest.InventorySHA256) != sha256.Size*2 {
		return domain.NewError(domain.ErrorPrecondition, "S3 manifest", "completion manifest inventory SHA-256 is required")
	}
	if _, err := hex.DecodeString(manifest.InventorySHA256); err != nil {
		return domain.WrapError(domain.ErrorPrecondition, "S3 manifest", "decode completion manifest inventory SHA-256", err)
	}
	if err := ValidatePath(manifest.Path); err != nil {
		return domain.WrapError(domain.ErrorPrecondition, "S3 manifest", "validate completion manifest path", err)
	}
	return nil
}
...
func (s *Store) readLock(ctx context.Context) (*Lock, string, error) {
	output, err := s.client.GetObject(ctx, &s3.GetObjectInput{Bucket: aws.String(s.config.Bucket), Key: aws.String(s.lockKey())})
	if err != nil {
		if isMissing(err) {
			return nil, "", nil
		}
