...
func TestPlanRejectsPersistedSessionOwnershipBeforeMutation(t *testing.T) {
	...
	plan, err := New(client, nil).Plan(context.Background(), Options{
		SessionID: "new-session", Operation: domain.OperationMigrate, SourceNamespace: "app", TemporaryNamespace: "system",
		StagingNamespace: "system", SessionNamespace: "system", SourcePVCs: []string{"data"}, TargetNode: "node-b", DestinationClass: "fast",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, check := range plan.Checks {
		if check.Name == "session-ownership" && (!strings.Contains(check.Message, old.ID) || !strings.Contains(check.Message, string(domain.PhaseCompleted)) || !strings.Contains(check.Message, "session status") || !strings.Contains(check.Message, "session cleanup") || !strings.Contains(check.Message, "--dry-run=false")) {
...
func TestPlanDiagnosesOrphanSessionOwnership(t *testing.T) {
	...
	if !strings.Contains(message, "orphan") || !strings.Contains(message, "cleanup-orphan") || !strings.Contains(message, "missing-session") {
...
func TestPlanRejectsConflictingSessionOwnership(t *testing.T) {
	...
	if plan.Ready || !hasFailedCheck(plan, "session-ownership") || !strings.Contains(ownershipMessage(plan), "conflicting") {
		t.Fatalf("plan=%#v", plan)
	}
}
