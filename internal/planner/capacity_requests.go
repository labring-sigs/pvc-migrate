...
func supportsDestinationCapacity(operation domain.Operation) bool {
	switch operation {
	case domain.OperationReserve, domain.OperationCopy, domain.OperationMigrate, domain.OperationMigratePod:
		return true
	default:
		return false
	}
...
func resolveDestinationCapacities(values []string, pvcNames []string) ([]string, error) {
...
func resolveNamedCapacity(values, pvcNames, result []string) ([]string, error) {
	...
	for _, raw := range values {
		...
		if _, exists := seen[key]; exists {
			return nil, fmt.Errorf("destination capacity for source PVC %q is specified more than once", key)
		}
		seen[key] = struct{}{}
		result[index] = value
	}
	if len(seen) != len(known) {
		...
		return nil, fmt.Errorf("destination capacity is missing source PVC mapping(s): %s", strings.Join(missing, ","))
	}
	return result, nil
}
...
func resolveDestinationPVCs(values, pvcNames []string) ([]string, error) {
	...
	if len(values) == 1 && !strings.Contains(values[0], "=") {
		if len(pvcNames) != 1 {
			return nil, fmt.Errorf("a bare --destination-pvc value is valid only for one source PVC; use source-pvc-name=destination-pvc-name mappings for multiple PVCs")
		}
	...
	for _, raw := range values {
		...
		key, value = strings.TrimSpace(key), strings.TrimSpace(value)
		if !ok || key == "" || value == "" {
			return nil, fmt.Errorf("named destination PVC %q must use source-pvc-name=destination-pvc-name", raw)
		}
		index, exists := known[key]
		if !exists {
		...
		if _, exists := seen[key]; exists {
			return nil, fmt.Errorf("destination PVC for source PVC %q is specified more than once", key)
		}
		seen[key] = struct{}{}
		result[index] = value
	}
	if len(seen) != len(known) {
		...
		sort.Strings(missing)
		return nil, fmt.Errorf("destination PVC is missing source PVC mapping(s): %s", strings.Join(missing, ","))
	}
	return result, nil
}
