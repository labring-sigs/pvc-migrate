...
func TestCheckPodDependenciesCollectsAllReferenceForms(t *testing.T) {
	...
	objects[0] = &corev1.ServiceAccount{
		ObjectMeta: metav1.ObjectMeta{Namespace: "app", Name: "runner"},
	}

	for _, name := range []string{"pull", "volume-secret", "projected-secret", "env-secret", "key-secret"} {
		objects = append(
			objects,
			&corev1.Secret{ObjectMeta: metav1.ObjectMeta{Namespace: "app", Name: name}},
		)
	}
	...
	for _, name := range []string{"volume-config", "projected-config", "env-config", "key-config"} {
		objects = append(
			objects,
			&corev1.ConfigMap{ObjectMeta: metav1.ObjectMeta{Namespace: "app", Name: name}},
		)
	}
	...
	New(
		...
		nil,
	).checkPodDependencies(context.Background(), plan, pod)

	if !plan.Ready || len(plan.Checks) != 1 ||
		!strings.Contains(
			plan.Checks[0].Message,
			"5 Secret(s), 4 ConfigMap(s), ServiceAccount/runner",
		) {
			...
			Volumes: []corev1.Volume{
				{
					...
					VolumeSource: corev1.VolumeSource{
						ConfigMap: &corev1.ConfigMapVolumeSource{
							LocalObjectReference: corev1.LocalObjectReference{Name: "a-config"},
						},
					},
				},
			},
...
func TestCheckPodDependenciesStopsOnKubernetesError(t *testing.T) {
	...
	if plan.Ready || len(plan.Checks) != 1 ||
		!strings.Contains(plan.Checks[0].Message, "API timeout") {
		t.Fatalf("dependency result: %#v", plan.Checks)
	}
}
