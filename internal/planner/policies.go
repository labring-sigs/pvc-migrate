...
func (p *Planner) checkNetworkPolicies(ctx context.Context, plan *domain.MigrationPlan, namespaces ...string) {
	...
	for _, namespace := range namespaces {
		...
		if len(policies.Items) > 0 {
			plan.AddCheck(warned("network-policy", fmt.Sprintf("namespace %s has %d NetworkPolicy object(s); clusterip copy connectivity will be validated by the real copy job", namespace, len(policies.Items))))
		} else {
