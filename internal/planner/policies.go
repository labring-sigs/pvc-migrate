...
func (p *Planner) checkLimitRanges(ctx context.Context, plan *domain.MigrationPlan, namespace string, volumes []domain.PlannedVolume) {
	...
	if err != nil {
		plan.AddCheck(failed("limit-range", fmt.Sprintf("list LimitRanges in %s: %v", namespace, err)))
		return
	}
	if items == nil {
		plan.AddCheck(failed("limit-range", fmt.Sprintf("list LimitRanges in %s returned an empty object", namespace)))
		return
	}
	...
	for _, limitRange := range items.Items {
		for _, item := range limitRange.Spec.Limits {
			...
			for _, volume := range volumes {
				...
				if parseErr != nil {
					violations = append(violations, fmt.Sprintf("%s has invalid capacity %q", volume.SourcePVC.Name, volume.Capacity))
					continue
				}
				if minimum, ok := item.Min[corev1.ResourceStorage]; ok && capacity.Cmp(minimum) < 0 {
					violations = append(violations, fmt.Sprintf("%s requires %s below %s minimum %s", volume.DestinationPVC.Name, capacity.String(), limitRange.Name, minimum.String()))
				}
				if maximum, ok := item.Max[corev1.ResourceStorage]; ok && capacity.Cmp(maximum) > 0 {
					violations = append(violations, fmt.Sprintf("%s requires %s above %s maximum %s", volume.DestinationPVC.Name, capacity.String(), limitRange.Name, maximum.String()))
				}
			}
		}
	}
	...
	plan.AddCheck(passed("limit-range", fmt.Sprintf("%d LimitRange object(s) permit all target PVC requests", len(items.Items))))
}
...
func (p *Planner) checkQuotas(ctx context.Context, plan *domain.MigrationPlan, namespace string, estimate domain.ResourceEstimate) {
	...
	if err != nil {
		plan.AddCheck(failed("resource-quota", fmt.Sprintf("list ResourceQuotas in %s: %v", namespace, err)))
		return
	}
	if items == nil {
		plan.AddCheck(failed("resource-quota", fmt.Sprintf("list ResourceQuotas in %s returned an empty object", namespace)))
		return
	}
	...
	for _, quota := range items.Items {
		...
		for name, requested := range demand {
			...
			if total.Cmp(limit) > 0 {
				violations = append(violations, fmt.Sprintf("%s/%s %s: used %s + requested %s exceeds hard %s", namespace, quota.Name, name, used.String(), requested.String(), limit.String()))
			}
		}
	}
	...
	plan.AddCheck(passed("resource-quota", fmt.Sprintf("%d ResourceQuota object(s), %d bounded resource(s), all have capacity", len(items.Items), checked)))
}
...
func quotaDemand(estimate domain.ResourceEstimate) (corev1.ResourceList, error) {
	...
	if err != nil {
		return nil, domain.WrapError(domain.ErrorInternal, "quota estimate", "parse storage demand", err)
	}
	...
	for class, value := range estimate.ByStorageClass {
		...
		if parseErr != nil {
			return nil, domain.WrapError(domain.ErrorInternal, "quota estimate", fmt.Sprintf("parse StorageClass %s demand", class), parseErr)
		}
...
func (p *Planner) checkNetworkPolicies(ctx context.Context, plan *domain.MigrationPlan, namespaces ...string) {
	...
	parallel.For(len(unique), func(index int) {
		...
		policies, err := p.client.NetworkingV1().NetworkPolicies(namespace).List(ctx, metav1.ListOptions{})
		count := 0
		if policies != nil {
			count = len(policies.Items)
		}
		results[index] = result{namespace: namespace, count: count, empty: policies == nil, err: err}
	})
	for _, result := range results {
		...
		if err != nil {
			plan.AddCheck(failed("network-policy", fmt.Sprintf("list NetworkPolicies in %s: %v", namespace, err)))
			continue
		}
		if result.empty {
			plan.AddCheck(failed("network-policy", fmt.Sprintf("list NetworkPolicies in %s returned an empty object", namespace)))
			continue
		}
		if result.count > 0 {
			plan.AddCheck(warned("network-policy", fmt.Sprintf("namespace %s has %d NetworkPolicy object(s); clusterip copy connectivity will be validated by the real copy job", namespace, result.count)))
		} else {
			plan.AddCheck(passed("network-policy", fmt.Sprintf("namespace %s has no NetworkPolicy objects", namespace)))
		}
	}
}
