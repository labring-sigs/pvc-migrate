...
func TestLoadPlanInventoryKeepsPVCPVAndStorageClassIndexes(t *testing.T) {
	...
	inventory := New(
		...
		nil,
	).loadPlanInventory(context.Background(), Options{SourceNamespace: "app", TargetNode: "node-b"}, []string{"data", "logs"}, false)
	...
	if inventory.storageClasses["fast"] == nil || inventory.storageClassError["fast"] != nil {
		t.Fatalf(
			"storage class result: classes=%v errors=%v",
			inventory.storageClasses,
			inventory.storageClassError,
		)
	}

	if inventory.targetNode == nil || inventory.targetNode.Name != "node-b" ||
		inventory.targetNodeErr != nil {
		t.Fatalf(
			"target node result: node=%v err=%v",
			inventory.targetNode,
			inventory.targetNodeErr,
		)
	}
}
...
func TestLoadPlanInventoryLoadsSourceAndExplicitDestinationClasses(t *testing.T) {
	...
	destinationClass := &storagev1.StorageClass{
		...
		Provisioner: "archive.example.io",
	}
	...
	inventory := New(
		plannerClient(objects...),
		nil,
	).loadPlanInventory(context.Background(), Options{
		SourceNamespace:  "app",
	...
	for _, name := range []string{"fast", destinationClass.Name} {
		if inventory.storageClasses[name] == nil || inventory.storageClassError[name] != nil {
			t.Fatalf(
				"StorageClass %s result: classes=%v errors=%v",
				name,
				inventory.storageClasses,
				inventory.storageClassError,
			)
