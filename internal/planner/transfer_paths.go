...
func resolveTransferScopes(
	sourceValues, destinationValues, pvcNames []string,
) ([]*domain.TransferScope, error) {
	sources, sourceSet, err := resolveTransferPathValues("source", sourceValues, pvcNames)
	if err != nil {
		return nil, err
	}

	destinations, destinationSet, err := resolveTransferPathValues(
		"destination",
		destinationValues,
		pvcNames,
	)
	...
	for index := range pvcNames {
		...
		if scopeErr != nil {
			return nil, fmt.Errorf(
				"transfer paths for source PVC %q are invalid: %w",
				pvcNames[index],
				scopeErr,
			)
		}
...
func resolveTransferPathValues(side string, values, pvcNames []string) ([]string, []bool, error) {
	...
	for _, raw := range values {
		...
		if !mapped {
			if len(pvcNames) != 1 {
				return nil, nil, fmt.Errorf(
					"a bare --%s-path value is valid only for one source PVC; use source-pvc-name=relative-path mappings for multiple PVCs",
					side,
				)
			}
		...
		if key == "" || value == "" {
			return nil, nil, fmt.Errorf(
				"--%s-path %q must use source-pvc-name=relative-path; use . for the PVC root",
				side,
				raw,
			)
		}
		...
		if set[index] {
			return nil, nil, fmt.Errorf(
				"--%s-path for source PVC %q is specified more than once",
				side,
				key,
			)
		}
		...
		if normalizeErr != nil {
			return nil, nil, fmt.Errorf(
				"--%s-path for source PVC %q is invalid: %w",
				side,
				key,
				normalizeErr,
			)
