...
func podMigrationIssues(spec corev1.PodSpec, sourceNode, targetNode string) []string {
	...
	if spec.SchedulerName != "" && spec.SchedulerName != "default-scheduler" {
		issues = append(
			issues,
			fmt.Sprintf(
				"custom scheduler %q cannot guarantee the requested target node",
				spec.SchedulerName,
			),
		)
	}
	...
	for _, volume := range spec.Volumes {
		...
		case volume.HostPath != nil && sourceNode != targetNode:
			issues = append(
				issues,
				fmt.Sprintf(
					"hostPath volume %q is node-local and source/target nodes differ",
					volume.Name,
				),
			)
		case volume.Ephemeral != nil:
			issues = append(
				issues,
				fmt.Sprintf("generic ephemeral volume %q is recreated empty", volume.Name),
			)
		}
	}
...
func schedulingIssues(spec corev1.PodSpec, node *corev1.Node) []string {
	...
	for _, taint := range node.Spec.Taints {
		...
		if !tolerates(spec.Tolerations, taint) {
			issues = append(
				issues,
				fmt.Sprintf("taint %s=%s:%s is untolerated", taint.Key, taint.Value, taint.Effect),
			)
		}
	}
	...
	for name, request := range requests {
		...
		if request.Cmp(allocatable) > 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"Pod resource request %s=%s exceeds node allocatable %s=%s",
					name,
					request.String(),
					name,
					allocatable.String(),
...
func selectorRequirementMatches(
