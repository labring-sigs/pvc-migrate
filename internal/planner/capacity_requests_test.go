...
func planWithDestinationCapacity(t *testing.T, capacities []string, allowShrink bool) *domain.MigrationPlan {
	t.Helper()
	return planWithDestinationCapacityObjects(t, plannerObjects("2Gi"), []string{"data"}, capacities, allowShrink)
}

func planWithDestinationCapacityObjects(t *testing.T, objects []runtime.Object, sourcePVCs, capacities []string, allowShrink bool) *domain.MigrationPlan {
	t.Helper()
	plan, err := New(plannerClient(objects...), nil).WithVolumeUsageReader(staticUsageReader{bytes: 1024}).Plan(context.Background(), Options{
...
func (p staticUsageReader) Read(context.Context, kube.VolumeUsageReadOptions) (kube.VolumeUsageReadResult, error) {
...
func TestPlanUsesRequestedDestinationCapacity(t *testing.T) {
	...
	if len(plan.Volumes) != 1 || plan.Volumes[0].SourceCapacity != "2Gi" || plan.Volumes[0].Capacity != "3Gi" {
	...
	if plan.TemporaryUsage.StorageRequests != "3Gi" || plan.TemporaryUsage.ByStorageClass["fast"] != "3Gi" {
		t.Fatalf("temporary usage=%#v", plan.TemporaryUsage)
	}
	if plan.RollbackRetention.StorageRequests != "2Gi" || plan.RollbackRetention.ByStorageClass["fast"] != "2Gi" {
...
func TestPlanDefaultsDestinationCapacityToSourcePVCapacity(t *testing.T) {
	...
	if plan.SessionSpec.Volumes[0].SourceCapacity != "2Gi" || plan.SessionSpec.Volumes[0].Capacity != "2Gi" {
...
func TestPlanAllowsExplicitDestinationShrinkWithWarning(t *testing.T) {
	...
	if plan.Volumes[0].SourceCapacity != "2Gi" || plan.Volumes[0].Capacity != "1Gi" || plan.TemporaryUsage.StorageRequests != "1Gi" {
		t.Fatalf("plan=%#v", plan)
	}
	found := false
	for _, check := range plan.Checks {
		if check.Name == "destination-capacity" && check.Passed && check.Severity == domain.SeverityWarning && strings.Contains(check.Message, "known to fit") {
			found = true
		}
	}
	if !found {
...
func (errorUsageReader) Read(context.Context, kube.VolumeUsageReadOptions) (kube.VolumeUsageReadResult, error) {
	return kube.VolumeUsageReadResult{}, fmt.Errorf("backend CRD has no usage field")
}

func TestPlanRejectsBackendUsageAboveShrinkTarget(t *testing.T) {
	plan, err := New(plannerClient(plannerObjects("2Gi")...), nil).WithVolumeUsageReader(staticUsageReader{bytes: 2 << 30}).Plan(context.Background(), Options{
		SessionID: "capacity-overflow", Operation: domain.OperationCopy, SourceNamespace: "app", TemporaryNamespace: "system", DestinationNamespace: "system", StagingNamespace: "system", SessionNamespace: "system", SourcePVCs: []string{"data"}, TargetNode: "node-b", DestinationClass: "fast", DestinationCapacities: []string{"1Gi"}, AllowVolumeShrink: true,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanRequiresExplicitSourceUsageSkip(t *testing.T) {
	base := Options{SessionID: "capacity-unknown", Operation: domain.OperationCopy, SourceNamespace: "app", TemporaryNamespace: "system", DestinationNamespace: "system", StagingNamespace: "system", SessionNamespace: "system", SourcePVCs: []string{"data"}, TargetNode: "node-b", DestinationClass: "fast", DestinationCapacities: []string{"1Gi"}, AllowVolumeShrink: true}
	plan, err := New(plannerClient(plannerObjects("2Gi")...), nil).WithVolumeUsageReader(errorUsageReader{}).Plan(context.Background(), base)
	if err != nil {
		t.Fatal(err)
	}
	if plan.Ready || !hasFailedCheck(plan, "source-usage") {
...
func TestPlanRequiresTrustedReaderByDefault(t *testing.T) {
	plan, err := New(plannerClient(plannerObjects("2Gi")...), nil).Plan(context.Background(), Options{
		SessionID: "capacity-no-reader", Operation: domain.OperationCopy, SourceNamespace: "app", TemporaryNamespace: "system", DestinationNamespace: "system", StagingNamespace: "system", SessionNamespace: "system", SourcePVCs: []string{"data"}, TargetNode: "node-b", DestinationClass: "fast", DestinationCapacities: []string{"1Gi"}, AllowVolumeShrink: true,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanKeepsCompleteDiagnosticsForInvalidCapacity(t *testing.T) {
	...
	if len(plan.Volumes) != 1 || plan.Volumes[0].Capacity != "2Gi" || plan.TemporaryUsage.StorageRequests != "2Gi" {
...
func TestPlanRejectsCapacityCountMismatchWithoutDroppingVolumes(t *testing.T) {
	...
	plan := planWithDestinationCapacityObjects(t, objects, []string{"data", "logs"}, []string{"data=3Gi"}, false)
	if plan.Ready || !hasFailedCheckContaining(plan, "destination-capacity", "missing source PVC mapping") {
		t.Fatalf("checks=%#v", plan.Checks)
	}
...
func TestPlanSeparatesDestinationUsageFromSourceRollbackRetention(t *testing.T) {
	objects := plannerObjects("2Gi")
	sourceClass := "slow"
	objects[5].(*corev1.PersistentVolumeClaim).Spec.StorageClassName = &sourceClass
	objects[6].(*corev1.PersistentVolume).Spec.StorageClassName = sourceClass
	...
	if plan.TemporaryUsage.ByStorageClass["fast"] != "3Gi" || plan.TemporaryUsage.PVCsByStorageClass["fast"] != 1 {
		t.Fatalf("temporary usage=%#v", plan.TemporaryUsage)
	}
	if plan.RollbackRetention.ByStorageClass[sourceClass] != "2Gi" || plan.RollbackRetention.PVCsByStorageClass[sourceClass] != 1 {
		t.Fatalf("rollback retention=%#v", plan.RollbackRetention)
	}
...
func TestResolveDestinationCapacitiesBroadcastsOrMatchesExplicitPVCNames(t *testing.T) {
	...
	if _, err := resolveDestinationCapacities([]string{"1Gi", "2Gi"}, []string{"data", "logs"}); err == nil || !strings.Contains(err.Error(), "pvc-name=capacity") {
		t.Fatalf("mismatch error=%v", err)
	}
}

func TestResolveDestinationPVCsRequiresCompleteExplicitMappings(t *testing.T) {
	got, err := resolveDestinationPVCs([]string{"logs=logs-new", "data=data-new"}, []string{"data", "logs"})
	...
	for _, values := range [][]string{
		...
		if got, err := resolveDestinationPVCs(values, []string{"data", "logs"}); err == nil || got != nil {
