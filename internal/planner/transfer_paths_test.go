...
func TestResolveTransferScopesSupportsSingleAndNamedPartialMappings(t *testing.T) {
	...
	if err != nil || len(single) != 1 || single[0] == nil || single[0].SourcePath != "data/mysql" || single[0].DestinationPath != "." {
	...
	if len(multiple) != 3 || multiple[0] == nil || multiple[0].SourcePath != domain.VolumeRootPath || multiple[0].DestinationPath != "restored/data" || multiple[1] == nil || multiple[1].SourcePath != "archive/current" || multiple[1].DestinationPath != domain.VolumeRootPath || multiple[2] != nil {
...
func TestPlanPersistsTransferScopeAndWarnsForOrchestratedMigration(t *testing.T) {
	plan, err := New(plannerClient(plannerObjects("2Gi")...), nil).Plan(context.Background(), Options{
		SessionID: "partial-path", Operation: domain.OperationMigrate,
		SourceNamespace: "app", TemporaryNamespace: "system", DestinationNamespace: "app", StagingNamespace: "system", SessionNamespace: "system",
		SourcePVCs: []string{"data"}, SourcePaths: []string{"data=mysql/current"}, DestinationPaths: []string{"data=restore/mysql"},
		TargetNode: "node-b", DestinationClass: "fast",
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(plan.Volumes) != 1 || plan.Volumes[0].TransferScope == nil || plan.Volumes[0].TransferScope.SourcePath != "mysql/current" || plan.Volumes[0].TransferScope.DestinationPath != "restore/mysql" {
		t.Fatalf("planned volumes=%#v", plan.Volumes)
	}
	if len(plan.SessionSpec.Volumes) != 1 || plan.SessionSpec.Volumes[0].TransferScope == nil || plan.SessionSpec.Volumes[0].TransferScope == plan.Volumes[0].TransferScope {
		t.Fatalf("session scope=%#v plan scope=%#v", plan.SessionSpec.Volumes, plan.Volumes)
	}
	foundWarning := false
	for _, check := range plan.Checks {
		if check.Name == "transfer-scope" && check.Severity == domain.SeverityWarning && strings.Contains(check.Message, "content outside") {
			foundWarning = true
		}
	}
	if !foundWarning {
...
func TestPartialSourceShrinkTreatsWholeVolumeUsageAsInconclusive(t *testing.T) {
	options := Options{
		SessionID: "partial-shrink", Operation: domain.OperationCopy,
		SourceNamespace: "app", TemporaryNamespace: "system", DestinationNamespace: "system", StagingNamespace: "system", SessionNamespace: "system",
		SourcePVCs: []string{"data"}, SourcePaths: []string{"data=selected"}, DestinationCapacities: []string{"1Gi"}, AllowVolumeShrink: true,
		TargetNode: "node-b", DestinationClass: "fast",
	}
	plan, err := New(plannerClient(plannerObjects("2Gi")...), nil).WithVolumeUsageReader(staticUsageReader{bytes: 1536 << 20}).Plan(context.Background(), options)
	if err != nil {
		t.Fatal(err)
	}
	if plan.Ready || !hasFailedCheckContaining(plan, "source-usage", "cannot prove that selected source directory") {
		t.Fatalf("checks=%#v", plan.Checks)
	}
