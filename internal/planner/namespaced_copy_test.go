...
func TestNamespacedCopyPlanningUsesOperationPlanDirectly(t *testing.T) {
	...
	report, err := New(client, nil).PlanNamespacedCopy(t.Context(), object, "example/tool:v1")
	...
	if _, err := New(
		client,
		nil,
	).PlanNamespacedCopy(t.Context(), object, "example/tool:v1"); domain.CategoryOf(
		err,
...
func TestNamespacedCopyRejectsExecutionStateBeforeDiscovery(t *testing.T) {
	for _, test := range []struct {
		name   string
		mutate func(*v1alpha1.Copy)
	}{
		{name: "runtime placement", mutate: func(object *v1alpha1.Copy) { object.Status.SourceNode = "worker" }},
		{name: "active pass", mutate: func(object *v1alpha1.Copy) { object.Status.Phase = v1alpha1.WorkflowPhase("WarmCopying") }},
		{name: "deleting", mutate: func(object *v1alpha1.Copy) { now := metav1.Now(); object.DeletionTimestamp = &now }},
	} {
		t.Run(test.name, func(t *testing.T) {
			client := plannerClient(plannerObjects("2Gi")...)
			object := &v1alpha1.Copy{ObjectMeta: metav1.ObjectMeta{Name: "copy", Namespace: "app"}}
			test.mutate(object)
			before := object.DeepCopy()

			_, err := New(client, nil).PlanNamespacedCopy(t.Context(), object, "example/tool:v1")
