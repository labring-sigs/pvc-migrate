...
func (p *Planner) PlanPodMigration(
	...
	if object == nil || object.Name == "" {
		return nil, domain.NewError(
			...
			"plan pod migration",
			"pod migration name is required",
		)
	}
	...
	if !migrationCanPlan(object.Status.WorkflowStatus, object.Status.Plan != nil,
		...
		return nil, domain.NewError(domain.ErrorPrecondition, "plan pod migration",
			"only an unplanned pod migration can be planned")
	}

	spec := object.Spec.DeepCopy()
	if err := validatePodMigrationSpec(spec.PodMigrationSpec); err != nil {
		return nil, err
	}
	...
	if report.Ready {
		...
		if clusterPlan.Workload.Adapter == v1alpha1.WorkloadStandalone &&
			clusterPlan.Workload.OriginalObject != nil {
			object.Status.OriginalPodSnapshotHash = kube.PodSnapshotHash(
				clusterPlan.Workload.OriginalObject.Raw,
			)
		}
	}
