...
func (p *Planner) loadStorageCapacity(
	ctx context.Context,
	mode domain.CapacityAwareness,
) *storageCapacityInventory {
	...
	items, err := p.client.StorageV1().
		CSIStorageCapacities(metav1.NamespaceAll).
	...
	if items == nil {
		inventory.err = errors.New("CSIStorageCapacity list returned an empty object")
		return inventory
	}
...
func capacityDemands(volumes []domain.PlannedVolume) (map[string]capacityDemand, error) {
	...
	for _, volume := range volumes {
		...
		if capacity.Sign() <= 0 {
			return nil, fmt.Errorf(
				"PVC %s/%s capacity %s must be positive",
				volume.SourcePVC.Namespace,
				volume.SourcePVC.Name,
				capacity.String(),
			)
...
) (known, unknown int, surplus resource.Quantity, compatible bool) {
...
func (inventory *storageCapacityInventory) evaluate(
	node *corev1.Node,
	demand capacityDemand,
) capacityEvaluation {
	report := domain.StorageCapacityReport{
	...
	classObjects := capacityObjectsForClass(inventory.items, demand.storageClass)

	report.PublishedObjects = len(classObjects)
	if len(classObjects) == 0 {
		report.Message = fmt.Sprintf(
			"StorageClass %s has no published CSIStorageCapacity object; backend capacity is unknown",
			demand.storageClass,
		)
	...
	matching, invalidTopology := matchingCapacityObjects(classObjects, node)

	report.MatchingObjects = len(matching)
	if len(matching) == 0 {
		if invalidTopology > 0 {
			report.Message = fmt.Sprintf(
				"StorageClass %s has %d invalid CSIStorageCapacity topology object(s); target node %s cannot be verified",
				demand.storageClass,
				invalidTopology,
				node.Name,
			)
		...
		report.Message = fmt.Sprintf(
			"no CSIStorageCapacity object for StorageClass %s matches target node %s",
			demand.storageClass,
			node.Name,
		)
	...
	maxCapacity, maxVolume, unknownCapacity, capacitySufficient, surplus := evaluateCapacityMatches(
		matching,
		demand,
	)
	...
	if capacitySufficient {
		...
		report.Message = fmt.Sprintf(
			"StorageClass %s reports at least %s available on target node %s for %d PVC(s)",
			demand.storageClass,
			report.ReportedCapacity,
			node.Name,
			demand.volumes,
		)
	...
	if unknownCapacity {
		report.Message = fmt.Sprintf(
			"StorageClass %s has no reported available capacity for target node %s",
			demand.storageClass,
			node.Name,
		)
		if report.MaximumVolumeSize != "" {
			report.Message += fmt.Sprintf(
				"; maximumVolumeSize=%s permits the largest request",
				report.MaximumVolumeSize,
			)
		}
	...
	report.Message = fmt.Sprintf(
		"StorageClass %s reports insufficient capacity for %s requested on target node %s",
		demand.storageClass,
		demand.total.String(),
		node.Name,
	)
	if report.MaximumVolumeSize != "" &&
		demand.largest.Cmp(resource.MustParse(report.MaximumVolumeSize)) > 0 {
		report.Message = fmt.Sprintf(
			"StorageClass %s maximumVolumeSize=%s is below the largest requested volume %s",
			demand.storageClass,
			report.MaximumVolumeSize,
			demand.largest.String(),
		)
	}
...
func capacityObjectsForClass(
	items []storagev1.CSIStorageCapacity,
	storageClass string,
) []storagev1.CSIStorageCapacity {
	objects := make([]storagev1.CSIStorageCapacity, 0)
	for _, item := range items {
		if item.StorageClassName == storageClass {
...
func matchingCapacityObjects(
	objects []storagev1.CSIStorageCapacity,
	node *corev1.Node,
) ([]storagev1.CSIStorageCapacity, int) {
	nodeLabels := labels.Set{}
	maps.Copy(nodeLabels, node.Labels)
	...
	return matching, invalidTopology
}

func evaluateCapacityMatches(
	matching []storagev1.CSIStorageCapacity,
	demand capacityDemand,
) (maxCapacity, maxVolume *resource.Quantity, unknown, sufficient bool, surplus resource.Quantity) {
	surplus = resource.MustParse("0")
	for _, item := range matching {
		...
		sufficient = true
		candidateSurplus := item.Capacity.DeepCopy()
	...
	return maxCapacity, maxVolume, unknown, sufficient, surplus
}
