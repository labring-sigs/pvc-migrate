...
func (p *Planner) loadPlanInventory(
	...
	for index, result := range inventory.pvcs {
		if result.err != nil || result.pvc == nil || result.pvc.Status.Phase != corev1.ClaimBound ||
			result.pvc.Spec.VolumeName == "" {
			continue
		}
	...
	dependentWG.Go(func() {
		parallel.For(len(pvIndexes), func(resultIndex int) {
			...
			inventory.pvs[index].pv, inventory.pvs[index].err = p.client.CoreV1().
				PersistentVolumes().
	...
	dependentWG.Go(func() {
		parallel.For(len(classes), func(index int) {
			...
			results[index].sc, results[index].err = p.client.StorageV1().
				StorageClasses().
				Get(ctx, name, metav1.GetOptions{})
		})
	})
