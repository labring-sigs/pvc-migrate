...
func TestSelectorRequirementMatchesOperators(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "numeric malformed value count",
			requirement: corev1.NodeSelectorRequirement{
				Key:      "size",
				Operator: corev1.NodeSelectorOpGt,
				Values:   []string{"9", "10"},
		...
		},
		{
			...
			requirement: corev1.NodeSelectorRequirement{Key: "zone", Operator: "Unknown"},
			actual:      "b",
			labels:      labels,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := selectorRequirementMatches(
				tt.requirement,
				tt.actual,
				tt.labels,
			); got != tt.want {
...
func TestSchedulingAcceptsAlternativeAffinityTermAndTolerations(t *testing.T) {
	node := &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name:   "node-b",
			Labels: map[string]string{"disk": "ssd", "zone": "b", "size": "10"},
		},
...
func TestResourceFitUsesInitContainerAndOverheadRequests(t *testing.T) {
	...
	if !known || len(issues) != 1 ||
		issues[0] != "Pod resource request cpu=2100m exceeds node allocatable cpu=2" {
...
func TestResourceFitReportsUnknownAllocatableResource(t *testing.T) {
	...
	issues, known := resourceFitIssues(
		spec,
		&corev1.Node{Status: corev1.NodeStatus{Allocatable: corev1.ResourceList{
			corev1.ResourceCPU: resource.MustParse("2"),
		}}},
	)
...
func TestToleratesWildcardExistsAndHonorsEffect(t *testing.T) {
	...
	if tolerates(
		[]corev1.Toleration{{Key: "dedicated", Operator: corev1.TolerationOpEqual, Value: "other"}},
		taint,
	) {
...
func TestAllowedTopologiesUseORBetweenTermsAndANDBetweenExpressions(t *testing.T) {
	node := &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name:   "node-b",
			Labels: map[string]string{"zone": "b", "rack": "2"},
		},
	}
	...
	sc := &storagev1.StorageClass{AllowedTopologies: []corev1.TopologySelectorTerm{
		{
			MatchLabelExpressions: []corev1.TopologySelectorLabelRequirement{
				{Key: "zone", Values: []string{"a"}},
			},
		},
		{
			MatchLabelExpressions: []corev1.TopologySelectorLabelRequirement{
				{Key: "zone", Values: []string{"b"}},
				{Key: "rack", Values: []string{"2"}},
			},
		},
	}}
