...
func collectPodDependencies(pod *corev1.Pod) []podDependency {
	...
	for _, volume := range pod.Spec.Volumes {
		collectVolumeDependencies(volume, secrets, configMaps)
	}
	...
	for _, container := range containers {
		collectContainerDependencies(container, secrets, configMaps)
	}
	...
	dependencies := make([]podDependency, 0, len(secrets)+len(configMaps)+1)
	for name := range secrets {
		if name != "" {
			dependencies = append(
				dependencies,
				podDependency{kind: dependencyKindSecret, name: name},
			)
		}
	}
	...
	for name := range configMaps {
		if name != "" {
			dependencies = append(
				dependencies,
				podDependency{kind: dependencyKindConfigMap, name: name},
			)
		}
	}
	...
	dependencies = append(
		dependencies,
		podDependency{kind: dependencyKindServiceAccount, name: serviceAccount},
	)

	sort.SliceStable(dependencies, func(i, j int) bool {
		if dependencies[i].kind != dependencies[j].kind {
			return dependencyKindOrder(
				dependencies[i].kind,
			) < dependencyKindOrder(
				dependencies[j].kind,
			)
...
func collectVolumeDependencies(
	volume corev1.Volume,
	secrets, configMaps map[string]struct{},
) {
...
func collectContainerDependencies(
...
func (p *Planner) checkPodDependencies(
	...
	dependencies := collectPodDependencies(pod)
	...
	parallel.For(len(dependencies), func(index int) {
		...
		case dependencyKindSecret:
			_, results[index] = p.client.CoreV1().
				Secrets(pod.Namespace).
				Get(ctx, item.name, metav1.GetOptions{})
	...
	secretCount, configMapCount := 0, 0
	...
	for _, dependency := range dependencies {
		...
		case dependencyKindConfigMap:
			configMapCount++
		case dependencyKindServiceAccount:
			serviceAccount = dependency.name
		}
	}
