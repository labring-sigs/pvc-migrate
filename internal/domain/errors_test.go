...
func TestErrorFormattingWrappingAndClassification(t *testing.T) {
	...
	if typed.Category != ErrorKubernetes || typed.Op != "load PVC" || typed.Message != "API request failed" {
...
func TestCategoryAndExitCodeDefaults(t *testing.T) {
	cases := []struct {
		...
		{name: "validation", err: NewError(ErrorValidation, "op", "message"), category: ErrorValidation, exitCode: 2},
		{name: "precondition", err: NewError(ErrorPrecondition, "op", "message"), category: ErrorPrecondition, exitCode: 3},
		{name: "conflict", err: NewError(ErrorConflict, "op", "message"), category: ErrorConflict, exitCode: 4},
		{name: "kubernetes", err: NewError(ErrorKubernetes, "op", "message"), category: ErrorKubernetes, exitCode: 5},
		{name: "copy", err: NewError(ErrorCopy, "op", "message"), category: ErrorCopy, exitCode: 6},
		{name: "timeout", err: NewError(ErrorTimeout, "op", "message"), category: ErrorTimeout, exitCode: 7},
		{name: "unknown category", err: NewError(ErrorCategory("future"), "op", "message"), category: ErrorCategory("future"), exitCode: 1},
	}
