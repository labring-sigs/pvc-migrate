...
func (s *recordingBackupSessionStore) Get(context.Context, string, string) (*domain.Session, error) {
...
func (l *recordingBackupSessionLock) Bind(ctx context.Context) (context.Context, context.CancelFunc) {
	l.bound = true
...
func (s *lockingBackupSessionStore) AcquireSessionLock(context.Context, string, string) (kube.SessionLock, error) {
	return s.lock, nil
}
...
func (m *recordingBackupOpenEBSManager) Shared(context.Context, domain.ObjectReference, domain.ObjectReference, string) (bool, error) {
...
func (m *recordingBackupOpenEBSManager) PrepareShared(context.Context, domain.ObjectReference) (kube.OpenEBSLVMSharedResult, error) {
	return m.prepared, nil
}

func (m *recordingBackupOpenEBSManager) EnableShared(_ context.Context, _ string, mount domain.OpenEBSLVMSharedMount) error {
	m.enabled = append(m.enabled, mount)
	return m.enableErr
}

func (m *recordingBackupOpenEBSManager) ValidateRestoreShared(_ context.Context, _ string, mount domain.OpenEBSLVMSharedMount) error {
	m.validatedRestores = append(m.validatedRestores, mount)
	return m.validateRestoreErr
}

func (m *recordingBackupOpenEBSManager) RestoreShared(_ context.Context, _ string, mount domain.OpenEBSLVMSharedMount) error {
...
func TestBuildBackupSessionIncludesMetadataWithoutCredentials(t *testing.T) {
	...
	if session.Spec.Type != domain.SessionTypeBackup || session.Spec.Backup == nil || !session.Spec.Backup.Online {
	...
	if session.Spec.Backup.SourcePVC.Name != pvc.Name || session.Spec.Backup.SourcePV.Name != pv.Name {
...
func TestBuildBackupSessionRejectsInvalidIDBeforePersistence(t *testing.T) {
	...
	if _, err := buildBackupSession(req, pvc, pv); domain.CategoryOf(err) != domain.ErrorValidation {
...
func TestLoadBackupCredentialsRecoversUncheckpointedSecret(t *testing.T) {
	...
	if _, err := loadBackupCredentials(context.Background(), client, session); domain.CategoryOf(err) != domain.ErrorConflict {
		t.Fatalf("category=%s error=%v", domain.CategoryOf(err), err)
	}
}
...
func TestPersistBackupCredentialsKeepsSecretWhenCheckpointResultIsUnknown(t *testing.T) {
	...
	if err := persistBackupCredentials(context.Background(), client, req, session); !errors.Is(err, updateErr) {
...
func TestRestoreBackupSharedMountsStopsPersistingAfterLeaseLoss(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), backupSessionLockContextKey{}, kube.SessionLock(lock))

	err := restoreBackupSharedMounts(ctx, Request{SessionStore: store, OpenEBSLVMManager: manager}, session)
	...
	if len(session.Status.OpenEBSLVMSharedMounts) != 1 {
		t.Fatalf("recovery checkpoint was cleared after Lease loss: %#v", session.Status.OpenEBSLVMSharedMounts)
	}
}
...
func TestFailBackupSessionDoesNotPersistAfterLeaseLoss(t *testing.T) {
	...
	if len(store.updated) != 0 || session.Status.Phase != "" {
		t.Fatalf("session changed after Lease loss: phase=%s updates=%d", session.Status.Phase, len(store.updated))
	}
}
...
func TestValidateBackupOpenEBSStateRequiresFlagForUnsharedActivePVC(t *testing.T) {
	manager := &recordingBackupOpenEBSManager{prepared: kube.OpenEBSLVMSharedResult{
		NeedsChange: true,
		LVMVolume:   domain.ObjectReference{Namespace: "openebs", Name: "lvm-data", UID: types.UID("lvm-uid")},
	}}
	info := &PVCInfo{
		PVC:       &corev1.PersistentVolumeClaim{ObjectMeta: metav1.ObjectMeta{Namespace: "app", Name: "data"}},
		PV:        &corev1.PersistentVolume{ObjectMeta: metav1.ObjectMeta{Name: "pv-data", UID: types.UID("pv-uid")}, Spec: corev1.PersistentVolumeSpec{CSI: &corev1.CSIPersistentVolumeSource{Driver: kube.OpenEBSLVMCSIDriver}}},
	...
	if err := validateBackupOpenEBSState(context.Background(), req, info); domain.CategoryOf(err) != domain.ErrorPrecondition {
...
func TestBackupSessionOpenEBSStatePersistsAndRestores(t *testing.T) {
	...
	if len(session.Status.OpenEBSLVMSharedMounts) != 1 || len(manager.enabled) != 1 || len(store.updated) != 1 {
		t.Fatalf("state was not persisted before enable: session=%#v enabled=%d updates=%d", session.Status.OpenEBSLVMSharedMounts, len(manager.enabled), len(store.updated))
	}
...
func TestBackupSessionDropsSharedRecoveryWhenEnableDidNotAcquireOwnership(t *testing.T) {
	...
	manager := &recordingBackupOpenEBSManager{
		prepared: kube.OpenEBSLVMSharedResult{
			NeedsChange: true, LVMVolume: domain.ObjectReference{Namespace: "openebs", Name: "lvm-data", UID: "lvm-uid"},
			PreviousShared: "no", PreviousSharedSet: true,
		},
		enableErr: domain.NewError(domain.ErrorConflict, "OpenEBS LVM shared mount", "ownership changed"),
	}
	info := &PVCInfo{
		PVC: &corev1.PersistentVolumeClaim{ObjectMeta: metav1.ObjectMeta{Namespace: "app", Name: "data"}},
		PV: &corev1.PersistentVolume{
			ObjectMeta: metav1.ObjectMeta{Name: "pv-data", UID: "pv-uid"},
			Spec:       corev1.PersistentVolumeSpec{CSI: &corev1.CSIPersistentVolumeSource{Driver: kube.OpenEBSLVMCSIDriver}},
		},
		Consumers: []string{"app/writer"},
	}
	req := Request{SessionStore: store, OpenEBSLVMManager: manager, OpenEBSLVMEnableShared: true}
	...
	if err := backupSessionOpenEBSState(context.Background(), req, session, info); domain.CategoryOf(err) != domain.ErrorConflict {
		t.Fatalf("category=%s error=%v", domain.CategoryOf(err), err)
	}
	if len(session.Status.OpenEBSLVMSharedMounts) != 0 || len(store.updated) != 2 {
		t.Fatalf("stale recovery state=%#v updates=%d", session.Status.OpenEBSLVMSharedMounts, len(store.updated))
	}
}
...
func TestBackupSessionRetainsSharedRecoveryAfterAmbiguousEnableErrorWhenOwned(t *testing.T) {
	...
	manager := &recordingBackupOpenEBSManager{
		...
		enableErr: domain.NewError(domain.ErrorKubernetes, "OpenEBS LVM shared mount", "connection lost"),
		shared:    true,
	}
	info := &PVCInfo{
		PVC: &corev1.PersistentVolumeClaim{ObjectMeta: metav1.ObjectMeta{Namespace: "app", Name: "data"}},
	...
	if err := backupSessionOpenEBSState(context.Background(), req, session, info); domain.CategoryOf(err) != domain.ErrorKubernetes {
		t.Fatalf("category=%s error=%v", domain.CategoryOf(err), err)
	}
	if len(session.Status.OpenEBSLVMSharedMounts) != 1 {
...
func TestBackupSessionResumesCheckpointBeforeSharedMountWasEnabled(t *testing.T) {
	conflictErr := domain.NewError(domain.ErrorConflict, "OpenEBS LVM shared mount", "temporary ownership is absent")
	...
	if len(manager.enabled) != 0 || len(manager.validatedRestores) != 1 {
		t.Fatalf("preflight enabled=%d restore validations=%d", len(manager.enabled), len(manager.validatedRestores))
	}
...
func TestBackupSessionRejectsCheckpointWithConflictingSharedMountOwnership(t *testing.T) {
	manager := &recordingBackupOpenEBSManager{
		sharedErr:          domain.NewError(domain.ErrorConflict, "OpenEBS LVM shared mount", "owned by another session"),
		validateRestoreErr: domain.NewError(domain.ErrorConflict, "restore OpenEBS LVM shared mount", "ownership changed"),
	}
	mount := domain.OpenEBSLVMSharedMount{
		SourcePV:  domain.ObjectReference{Kind: "PersistentVolume", Name: "pv-data", UID: "pv-uid"},
		LVMVolume: domain.ObjectReference{Namespace: "openebs", Name: "lvm-data", UID: "lvm-uid"},
	...
	if err := validateBackupOpenEBSState(context.Background(), req, info); domain.CategoryOf(err) != domain.ErrorConflict {
		t.Fatalf("category=%s error=%v", domain.CategoryOf(err), err)
	}
...
func TestResumeCompletesMatchingPublishedBackupWithoutRunningTool(t *testing.T) {
	...
	if _, err := client.CoreV1().Pods(pvc.Namespace).Create(context.Background(), staleProbe, metav1.CreateOptions{}); err != nil {
		t.Fatal(err)
	}
	if _, err := client.CoreV1().Pods(pvc.Namespace).Create(context.Background(), foreignProbe, metav1.CreateOptions{}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := client.CoreV1().Pods(pvc.Namespace).Get(context.Background(), staleProbe.Name, metav1.GetOptions{}); err != nil {
		t.Fatalf("dry-run deleted stale backup probe: %v", err)
	}
	if err := Resume(context.Background(), client, resumeReq, session); err != nil {
		t.Fatal(err)
	}
	if _, err := client.CoreV1().Pods(pvc.Namespace).Get(context.Background(), staleProbe.Name, metav1.GetOptions{}); !apierrors.IsNotFound(err) {
		t.Fatalf("stale backup probe remains: %v", err)
	}
	if _, err := client.CoreV1().Pods(pvc.Namespace).Get(context.Background(), foreignProbe.Name, metav1.GetOptions{}); err != nil {
	...
	if err := kube.DeleteBackupCredentialsSecret(context.Background(), client, secretRef, session.ID); err != nil {
		t.Fatal(err)
	}
...
func TestValidateResumeRejectsPublishedBackupFromAnotherSession(t *testing.T) {
	...
	pv, _ := client.CoreV1().PersistentVolumes().Get(context.Background(), "pv-data", metav1.GetOptions{})
	session, err := buildBackupSession(req, pvc, pv)
	if err != nil {
		t.Fatal(err)
	}
	...
	manifest := objectstore.Manifest{
		Version: 2, CreatedAt: time.Now().UTC(), Bucket: "backups", Prefix: "pv-migrate", Name: "daily",
		SessionID: "another-session", SourceNamespace: "default", SourcePVC: "data", SourcePVCUID: "pvc",
		SourcePV: "pv-data", SourcePVUID: "pv", Capacity: "1Gi", VolumeMode: "Filesystem",
		Consistency: backupConsistency(false), Compression: "none",
		InventorySHA256: "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
	}
	storeAPI.manifest, err = json.Marshal(manifest)
	if err != nil {
	...
	resumeReq := Request{
		SessionStore: sessionStore,
		ObjectStoreFactory: func(_ context.Context, config objectstore.Config) (*objectstore.Store, error) {
			return objectstore.NewWithClient(storeAPI, config, objectstore.Credentials{AccessKey: config.AccessKey, SecretKey: config.SecretKey})
		},
	}
	if err := ValidateResume(context.Background(), client, resumeReq, session); domain.CategoryOf(err) != domain.ErrorConflict {
		t.Fatalf("category=%s error=%v", domain.CategoryOf(err), err)
	}
	if err := Resume(context.Background(), client, resumeReq, session); domain.CategoryOf(err) != domain.ErrorConflict {
