...
func (f *podMigrationFlags) bind(command *cobra.Command) {
	...
	flags.StringVar(&f.sourceNode, "source-node", "", "Source tool node; inferred from the selected Pod")
	...
	flags.BoolVar(&f.openEBSLVMEnableShared, "openebs-lvm-enable-shared", false, "Enable same-node shared mounts for OpenEBS LVM warm copy and multi-consumer RWO destinations")
	flags.StringVar(&f.podName, "pod", "", "Stateful Pod migration unit")
	flags.StringVar(&f.switchoverCandidate, "kubeblocks-candidate", "", "Switchover target for a supported InstanceSet-backed KubeBlocks primary")
	flags.BoolVar(&f.allowLeaderDowntime, "allow-leader-downtime", false, "Acknowledge selected leader downtime for InstanceSet-backed KubeBlocks or native StatefulSet scale-down")
}
...
func (f *podMigrationFlags) planOptions(
	...
	if err := validateDestinationCapacityFlags(domain.OperationMigratePod, false, f.destinationCapacities, f.allowVolumeShrink, f.skipSourceUsageCheck, f.sourcePaths, f.destinationPaths); err != nil {
...
func (r *rootState) newMigratePodCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			if err := validateDestinationCapacityFlags(domain.OperationMigratePod, false, flags.destinationCapacities, flags.allowVolumeShrink, flags.skipSourceUsageCheck, flags.sourcePaths, flags.destinationPaths); err != nil {
			...
			if flags.precopyPasses < 0 {
				return domain.NewError(domain.ErrorValidation, "migrate-pod", "--precopy-passes cannot be negative")
			}
...
func (r *rootState) newPodMigrationPlanCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if err := validateDestinationCapacityFlags(domain.OperationMigratePod, existing, flags.destinationCapacities, flags.allowVolumeShrink, flags.skipSourceUsageCheck, flags.sourcePaths, flags.destinationPaths); err != nil {
				return err
			}
			...
			if flags.precopyPasses < 0 {
				return domain.NewError(domain.ErrorValidation, "migrate-pod plan", "--precopy-passes cannot be negative")
			}
			...
			if existing {
				...
				if session.Spec.Operation() != domain.OperationMigratePod {
					return reportSessionError(cmd, session, domain.NewError(domain.ErrorPrecondition, "migrate-pod plan", "migrate-pod plan requires a MigratePod session"))
				}
