...
func (r *rootState) newCopyStatusCommand() *cobra.Command {
	return &cobra.Command{
		Use: "status [SESSION]", Short: "Show one copy session or list all copy sessions", Args: cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer cancel()
			if len(args) == 1 {
				session, err := r.workflowSession(ctx, runtime, cmd, args[0], domain.SessionTypeCopy, "copy status")
				if err != nil {
					return err
				}
...
func (r *rootState) newCopyResumeCommand() *cobra.Command {
	...
	command := &cobra.Command{
		Use: "resume SESSION", Short: "Continue a copy session from its persisted phase", Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			runtime, err := r.runtime()
			if err != nil {
				return err
			...
			ctx, cancel := r.context(cmd.Context())
			defer cancel()
			session, err := r.workflowSession(ctx, runtime, cmd, args[0], domain.SessionTypeCopy, "copy resume")
			if err != nil {
				return err
			}
			if dryRun {
...
func (r *rootState) newCopyAbortCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ctx, cancel := r.context(cmd.Context())
			defer cancel()
			session, err := r.workflowSession(ctx, runtime, cmd, args[0], domain.SessionTypeCopy, "copy abort")
			if err != nil {
				return err
			}
			if dryRun {
...
func (r *rootState) newCopyCleanupCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ctx, cancel := r.context(cmd.Context())
			defer cancel()
			session, err := r.workflowSession(ctx, runtime, cmd, args[0], domain.SessionTypeCopy, "copy cleanup")
			if err != nil {
				return err
			}
			if dryRun {
