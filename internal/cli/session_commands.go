	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if options.DeleteSession {
				_, err := fmt.Fprintf(cmd.ErrOrStderr(), "session %s cleanup completed; the session ConfigMap and Lease were deleted, and active workload storage was preserved\n", args[0])
				return err
			}
			return printSessionResult(cmd, runtime, session)
		},
...
func (r *rootState) newSessionOrphanCleanupCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, err = fmt.Fprintf(cmd.ErrOrStderr(), "orphan ownership for session %s was cleared; the session ConfigMap was already absent\n", args[0])
			return err
		},
	}
