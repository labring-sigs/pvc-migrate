	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 1 {
				...
				if err := requireCLISessionType(session, domain.SessionTypeMigrate, "migrate status"); err != nil {
			...
			return writeSessionListGuidance(cmd.ErrOrStderr(), r.global.sessionNamespace, sessions, sessionCommandPrefixForCommand(cmd, r.global.sessionNamespace), "migrate")
		},
	}
}
...
func (r *rootState) newOfflineMigrationResumeCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := requireCLISessionType(session, domain.SessionTypeMigrate, "migrate resume"); err != nil {
				return reportSessionError(cmd, session, err)
			}
			if dryRun {
...
func (r *rootState) newOfflineMigrationAbortCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := requireCLISessionType(session, domain.SessionTypeMigrate, "migrate abort"); err != nil {
				return reportSessionError(cmd, session, err)
			}
			if dryRun {
...
func (r *rootState) newOfflineMigrationRollbackCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := requireCLISessionType(session, domain.SessionTypeMigrate, "migrate rollback"); err != nil {
				return reportSessionError(cmd, session, err)
			}
			if dryRun {
				if err := runtime.service.ValidateOfflineMigrationRollback(ctx, session); err != nil {
					return reportSessionError(cmd, session, err)
				}
				return printSessionResult(cmd, runtime, session)
			}
...
func (r *rootState) newOfflineMigrationCleanupCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := requireCLISessionType(session, domain.SessionTypeMigrate, "migrate cleanup"); err != nil {
				return reportSessionError(cmd, session, err)
			}
			if dryRun {
				if err := runtime.service.ValidateOfflineMigrationCleanup(ctx, session, options); err != nil {
					return reportSessionError(cmd, session, err)
				}
				return printSessionResult(cmd, runtime, session)
			}
