...
func (f *migrationFlags) bind(
	command *cobra.Command,
	includePod, includeSourceNode, includeController, includePrecopy bool,
) {
	flags := command.Flags()
	...
	if includePrecopy {
		flags.IntVar(
			...
			"precopy-passes",
			1,
			"Warm-copy passes before workload pause",
		)
		flags.BoolVar(
...
func (f *migrationFlags) planOptions(
	state *rootState,
	operation domain.Operation,
	useTemporary bool,
) (planner.Options, error) {
...
func (r *rootState) newMigrationPlanCommand(
	operation domain.Operation,
	useTemporary, includePod, includeSourceNode, includeController, includePrecopy, includeOnline bool,
) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if (operation == domain.OperationMigrate || operation == domain.OperationMigratePod) &&
				flags.destinationNamespace != "" &&
				flags.destinationNamespace != flags.sourceNamespace {
				return domain.NewError(
					domain.ErrorPrecondition,
					"plan",
					"an orchestrated migration keeps application PVCs in the source namespace",
				)
			}
			...
			if targetsExistingSession(flags) &&
				(operation == domain.OperationReserve || operation == domain.OperationCopy) {
	...
	if includeOnline {
		command.Flags().
			BoolVar(&flags.online, "online", false, "Allow active PVC consumers for one finite warm-copy pass")
	}
...
func (r *rootState) newReserveCommand() *cobra.Command {
	...
	command.AddCommand(
		r.newMigrationPlanCommand(domain.OperationReserve, true, true, false, false, false, false),
	)
...
func (r *rootState) newCopyCommand() *cobra.Command {
	...
	bindDryRun(command, &dryRun)
	command.AddCommand(
		r.newMigrationPlanCommand(domain.OperationCopy, false, true, true, false, false, true),
	)
...
func validateDestinationCapacityFlags(
	...
	if existingSession &&
		(len(flags.destinationCapacities) > 0 || flags.allowVolumeShrink || flags.skipSourceUsageCheck) {
		return domain.NewError(
			domain.ErrorValidation,
			string(operation),
...
func (r *rootState) newSessionFlagPlanCommand(
	action string,
