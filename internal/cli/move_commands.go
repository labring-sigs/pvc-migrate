	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if destinationNamespace == "" {
				return domain.NewError(domain.ErrorValidation, "move", "--destination-namespace is required")
			}
			if destinationPVC == "" {
...
func (r *rootState) newMovePlanCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			if sourcePVC == "" {
				return domain.NewError(domain.ErrorValidation, "move plan", "--source-pvc is required")
			}
			if destinationNamespace == "" {
				return domain.NewError(domain.ErrorValidation, "move plan", "--destination-namespace is required")
			}
			if destinationPVC == "" {
				destinationPVC = sourcePVC
			}
