...
func (r *rootState) newRenameStatusCommand() *cobra.Command {
	return &cobra.Command{
		Use: "status [SESSION]", Short: "Show one rename session or list all rename sessions", Args: cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer cancel()
			if len(args) == 1 {
				session, err := r.workflowSession(ctx, runtime, cmd, args[0], domain.SessionTypeRename, "rename status")
				if err != nil {
					return err
				}
...
func (r *rootState) newRenameResumeCommand() *cobra.Command {
	...
	command := &cobra.Command{
		Use: "resume SESSION", Short: "Continue a rename session from its persisted phase", Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			runtime, err := r.runtime()
			if err != nil {
				return err
			...
			ctx, cancel := r.context(cmd.Context())
			defer cancel()
			session, err := r.workflowSession(ctx, runtime, cmd, args[0], domain.SessionTypeRename, "rename resume")
			if err != nil {
				return err
			}
			if dryRun {
...
func (r *rootState) newRenameAbortCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ctx, cancel := r.context(cmd.Context())
			defer cancel()
			session, err := r.workflowSession(ctx, runtime, cmd, args[0], domain.SessionTypeRename, "rename abort")
			if err != nil {
				return err
			}
			if dryRun {
...
func (r *rootState) newRenameRollbackCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ctx, cancel := r.context(cmd.Context())
			defer cancel()
			session, err := r.workflowSession(ctx, runtime, cmd, args[0], domain.SessionTypeRename, "rename rollback")
			if err != nil {
				return err
			}
			if dryRun {
...
func (r *rootState) newRenameCleanupCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ctx, cancel := r.context(cmd.Context())
			defer cancel()
			session, err := r.workflowSession(ctx, runtime, cmd, args[0], domain.SessionTypeRename, "rename cleanup")
			if err != nil {
				return err
			}
			if dryRun {
