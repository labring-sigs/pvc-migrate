...
func (r *rootState) newRenameCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if destinationPVC == "" {
				return domain.NewError(domain.ErrorValidation, "rename", "--destination-pvc is required")
			}
			if sessionID == "" {
...
func (r *rootState) newRenamePlanCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			if sourcePVC == "" {
				return domain.NewError(domain.ErrorValidation, "rename plan", "--source-pvc is required")
			}
			if destinationPVC == "" {
				return domain.NewError(domain.ErrorValidation, "rename plan", "--destination-pvc is required")
			}
			if sessionID == "" {
				generated, err := domain.NewSessionID(time.Now())
				if err != nil {
