...
func testCopyBackupRestoreFlags(t *testing.T, root *cobra.Command) {
	...
	if label, value := transferResultIdentity(false, "backup-1"); label != "session" || value != "backup-1" {
		t.Fatalf("backup result identity=%q/%q", label, value)
	}
	if label, value := transferResultIdentity(true, "restore-1"); label != "operation-id" || value != "restore-1" {
