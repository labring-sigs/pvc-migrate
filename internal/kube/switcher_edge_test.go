...
func TestVerifyVolumeOfflineChecksSourceAndDestinationConsumers(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := client.CoreV1().Pods(test.namespace).Create(context.Background(), &corev1.Pod{
	...
	if err := NewSwitcher(fake.NewClientset()).VerifyVolumeOffline(context.Background(), volume); err == nil {
	...
	tests := []struct {
	...
	}{
		{
			...
			mutate: func(ctx context.Context, switcher *Switcher, volume *domain.VolumeSpec) error {
				pvc, err := switcher.client.CoreV1().PersistentVolumeClaims(volume.SourcePVC.Namespace).Get(ctx, volume.SourcePVC.Name, metav1.GetOptions{})
				if err != nil {
					return err
				}
				pvc.UID = "replacement-pvc"
				_, err = switcher.client.CoreV1().PersistentVolumeClaims(pvc.Namespace).Update(ctx, pvc, metav1.UpdateOptions{})
				return err
			},
		},
		{
			...
			category: domain.ErrorConflict,
			mutate: func(ctx context.Context, switcher *Switcher, volume *domain.VolumeSpec) error {
				pvc, err := switcher.client.CoreV1().PersistentVolumeClaims(volume.DestinationPVC.Namespace).Get(ctx, volume.DestinationPVC.Name, metav1.GetOptions{})
				if err != nil {
					return err
				}
				pvc.UID = "replacement-pvc"
		...
		},
		{
			...
			mutate: func(ctx context.Context, switcher *Switcher, volume *domain.VolumeSpec) error {
				...
				_, err = switcher.client.CoreV1().PersistentVolumeClaims(pvc.Namespace).UpdateStatus(ctx, pvc, metav1.UpdateOptions{})
				return err
			},
		},
		{
		...
		},
		{
			...
			mutate: func(ctx context.Context, switcher *Switcher, volume *domain.VolumeSpec) error {
				...
				_, err = switcher.client.CoreV1().PersistentVolumes().Update(ctx, pv, metav1.UpdateOptions{})
				return err
			},
		},
		{
...
func TestEnsureDetachedWaitsForCSIAttachmentState(t *testing.T) {
	attachment := &storagev1.VolumeAttachment{
		...
		Spec:       storagev1.VolumeAttachmentSpec{Source: storagev1.VolumeAttachmentSource{PersistentVolumeName: stringPointer("pv-data")}},
...
func TestDeletePVCClassifiesPreconditionConflict(t *testing.T) {
	...
	client.PrependReactor("delete", "persistentvolumeclaims", func(clienttesting.Action) (bool, runtime.Object, error) {
		return true, nil, apierrors.NewConflict(schema.GroupResource{Resource: "persistentvolumeclaims"}, pvc.Name, errors.New("resource version changed"))
	})
	err := NewSwitcher(client).deletePVC(context.Background(), domain.ObjectReference{Namespace: pvc.Namespace, Name: pvc.Name, UID: pvc.UID})
...
func TestMarkPVPairRejectsForeignSessionOwnership(t *testing.T) {
	...
	source, getErr := client.CoreV1().PersistentVolumes().Get(context.Background(), "source", metav1.GetOptions{})
	if getErr != nil {
		t.Fatal(getErr)
	}
...
func TestMarkPVPairAllowsMissingRollbackPV(t *testing.T) {
	...
	if err := NewSwitcher(client).markPVPair(context.Background(), "session", volume, true); err != nil {
		t.Fatal(err)
	}
	source, err := client.CoreV1().PersistentVolumes().Get(context.Background(), "source", metav1.GetOptions{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestMarkPVPairRejectsMissingSourceDuringActivation(t *testing.T) {
	...
	if err := NewSwitcher(client).markPVPair(context.Background(), "session", volume, false); err == nil {
...
func TestActivateVolumeValidatesPreconditionsBeforeMutation(t *testing.T) {
	tests := []struct {
		...
		{name: "final sync missing", mutate: func(_ *domain.VolumeSpec, status *domain.VolumeStatus) { status.Sync.FinalCompletedAt = nil }},
		{name: "destination PV name missing", mutate: func(volume *domain.VolumeSpec, _ *domain.VolumeStatus) { volume.DestinationPV.Name = "" }},
		{name: "destination PV UID missing", mutate: func(volume *domain.VolumeSpec, _ *domain.VolumeStatus) { volume.DestinationPV.UID = "" }},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, getErr := switcher.client.CoreV1().PersistentVolumeClaims("app").Get(context.Background(), "data", metav1.GetOptions{}); getErr != nil {
...
func TestActivateVolumeResumesAtEveryProgressBoundary(t *testing.T) {
	for failurePoint := 1; failurePoint <= 3; failurePoint++ {
		t.Run(fmt.Sprintf("progress-%d", failurePoint), func(t *testing.T) {
			...
			if active.Spec.VolumeName != volume.DestinationPV.Name || status.Activation.ActivatedAt == nil {
...
func TestRollbackVolumeResumesAfterProgressFailure(t *testing.T) {
	...
	if err := switcher.RollbackVolume(ctx, session, volume, status, func() error { return sentinel }); !errors.Is(err, sentinel) {
...
func TestRollbackVolumeFastPathRequiresSourcePVCIdentityOrSessionOwnership(t *testing.T) {
	tests := []struct {
		...
		{name: "recovered PVC owned by session", uid: types.UID("recovered-pvc-uid"), annotate: true},
		{name: "replacement PVC without ownership", uid: types.UID("foreign-pvc-uid"), wantError: domain.ErrorConflict},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			ctx := context.Background()
...
func TestDeletePVCUsesUIDAndResourceVersionPreconditions(t *testing.T) {
	...
	switcher.poll = time.Millisecond
	if err := switcher.deletePVC(ctx, domain.ObjectReference{Namespace: "app", Name: "data", UID: uid}); err != nil {
		t.Fatal(err)
	}
	if preconditions == nil || preconditions.UID == nil || *preconditions.UID != uid || preconditions.ResourceVersion == nil || *preconditions.ResourceVersion != "17" {
...
func TestDeletePVCDetectsNameReuseDuringDeletion(t *testing.T) {
	...
	client.PrependReactor("delete", "persistentvolumeclaims", func(action clienttesting.Action) (bool, runtime.Object, error) {
		...
		replacement := &corev1.PersistentVolumeClaim{ObjectMeta: metav1.ObjectMeta{
			Namespace: "app", Name: "data", UID: types.UID("replacement"), ResourceVersion: "18",
		}}
...
func TestReservePVIdempotencyAndConflictChecks(t *testing.T) {
	...
	t.Run("stale claim UID is cleared", func(t *testing.T) {
		...
		if pv.Spec.ClaimRef == nil || pv.Spec.ClaimRef.UID != "" || pv.Spec.ClaimRef.Namespace != "app" || pv.Spec.ClaimRef.Name != "data" {
	...
	for _, test := range []struct {
		...
		},
		{
			name: "UID changed",
			ref:  ref,
			pv: &corev1.PersistentVolume{ObjectMeta: metav1.ObjectMeta{Name: "pv", UID: types.UID("replacement")},
				Status: corev1.PersistentVolumeStatus{Phase: corev1.VolumeReleased}},
		},
	} {
		t.Run(test.name, func(t *testing.T) {
...
func TestEnsureRetainPreservesPolicyAndRejectsIdentityConflicts(t *testing.T) {
	...
	err = switcher.ensureRetain(ctx, domain.ObjectReference{Name: foreign.Name, UID: foreign.UID}, "session", "source")
	if domain.CategoryOf(err) != domain.ErrorConflict {
		t.Fatalf("foreign owner category=%s error=%v", domain.CategoryOf(err), err)
	}
	err = switcher.ensureRetain(ctx, domain.ObjectReference{Name: ref.Name, UID: types.UID("replacement")}, "session", "source")
	if domain.CategoryOf(err) != domain.ErrorConflict {
...
func TestVerifyBindingValidatesBothSidesOfBinding(t *testing.T) {
	...
	pvc, pv, ref := base()
	if err := NewSwitcher(fake.NewClientset(pv)).verifyBinding(context.Background(), pvc, ref); err != nil {
	...
	tests := []struct {
		...
		{"PVC pending", func(pvc *corev1.PersistentVolumeClaim, _ *corev1.PersistentVolume, _ *domain.ObjectReference) {
			pvc.Status.Phase = corev1.ClaimPending
		}},
		{"PVC bound elsewhere", func(pvc *corev1.PersistentVolumeClaim, _ *corev1.PersistentVolume, _ *domain.ObjectReference) {
			pvc.Spec.VolumeName = "other"
		}},
		{"PV UID changed", func(_ *corev1.PersistentVolumeClaim, pv *corev1.PersistentVolume, _ *domain.ObjectReference) {
			pv.UID = "replacement"
		}},
		{"claimRef absent", func(_ *corev1.PersistentVolumeClaim, pv *corev1.PersistentVolume, _ *domain.ObjectReference) {
			pv.Spec.ClaimRef = nil
		}},
		{"claimRef PVC UID changed", func(_ *corev1.PersistentVolumeClaim, pv *corev1.PersistentVolume, _ *domain.ObjectReference) {
			pv.Spec.ClaimRef.UID = "replacement"
		}},
		{"claimRef namespace changed", func(_ *corev1.PersistentVolumeClaim, pv *corev1.PersistentVolume, _ *domain.ObjectReference) {
			pv.Spec.ClaimRef.Namespace = "other"
		}},
		{"claimRef name changed", func(_ *corev1.PersistentVolumeClaim, pv *corev1.PersistentVolume, _ *domain.ObjectReference) {
...
func TestCreateAndValidateActivePVCRejectUnexpectedObjects(t *testing.T) {
	...
	validationClient.PrependReactor("create", "persistentvolumeclaims", func(action clienttesting.Action) (bool, runtime.Object, error) {
	...
	err = NewSwitcher(validationClient).validateActivePVC(context.Background(), session, volume, volume.DestinationPV, volume.StorageClass)
	if domain.CategoryOf(err) != domain.ErrorPrecondition {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.progressFail {
				...
				if _, err := switcher.RenamePVC(ctx, session, volume, func() error { return sentinel }); !errors.Is(err, sentinel) {
			...
			if pv.Labels[ResourceRoleLabel] != "active" || pv.Spec.ClaimRef == nil || pv.Spec.ClaimRef.UID != renamed.UID {
...
func TestRenamePVCRejectsExistingDestination(t *testing.T) {
	switcher, session, volume := configureRenameFixture(t)
	client := switcher.client.(*fake.Clientset)
	err := client.Tracker().Create(corev1.SchemeGroupVersion.WithResource("persistentvolumeclaims"), &corev1.PersistentVolumeClaim{
		ObjectMeta: metav1.ObjectMeta{Namespace: "app", Name: "data-renamed", UID: types.UID("foreign")},
		Spec:       corev1.PersistentVolumeClaimSpec{VolumeName: "other-pv"},
	}, "app")
	if err != nil {
		t.Fatal(err)
