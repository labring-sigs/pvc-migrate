...
func TestReservationToolPodUsesZeroResourceQuotaFootprint(t *testing.T) {
	...
	if got := created.Spec.Containers[0].Command; len(got) != 3 || got[0] != "sh" ||
		got[1] != "-c" ||
		got[2] != "test -d /data && exec sleep 3600" {
	...
	if len(resources.Requests) != 3 || len(resources.Limits) != 2 {
		t.Fatalf(
			"unexpected resource keys: requests=%#v limits=%#v",
			resources.Requests,
			resources.Limits,
		)
	}
...
func TestPVSupportsNodeChecksRequiredTopology(t *testing.T) {
	...
	if !PVSupportsNode(pv, matching) || PVSupportsNode(pv, other) {
		t.Fatalf(
			"topology matching=%t other=%t",
			PVSupportsNode(pv, matching),
			PVSupportsNode(pv, other),
		)
	}
}
...
func TestReservationKeepsPVCStorageRequestSeparateFromToolResources(t *testing.T) {
	...
	client.PrependReactor(
		...
		func(action clienttesting.Action) (bool, runtime.Object, error) {
			...
			pvcObject, err := client.Tracker().
				Get(corev1.SchemeGroupVersion.WithResource("persistentvolumeclaims"), volume.DestinationPVC.Namespace, volume.DestinationPVC.Name)
			if err != nil {
				return true, nil, err
			}
			...
			if err := client.Tracker().
				Update(corev1.SchemeGroupVersion.WithResource("persistentvolumeclaims"), pvc, pvc.Namespace); err != nil {
				return true, nil, err
			}
	...
	if len(pvc.Spec.Resources.Requests) != 1 || len(pvc.Spec.Resources.Limits) != 0 {
		t.Fatalf(
			"PVC resource footprint changed: requests=%#v limits=%#v",
