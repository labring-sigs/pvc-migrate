...
func TestServiceAccountIdentityFingerprintStableAcrossMapOrder(t *testing.T) {
	...
	second.Annotations = map[string]string{
		"audience": "s3",
		"role":     "arn:aws:iam::123:role/transfer",
	}
	if got, want := ServiceAccountIdentityFingerprint(
		first,
	), ServiceAccountIdentityFingerprint(
		second,
	); got != want ||
		got == "" {
...
func TestServiceAccountIdentityFingerprintIgnoresRotatingFields(t *testing.T) {
	base := &corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{
		...
		Annotations: map[string]string{
			"cloud.google.com/iam-service-account": "transfer@example",
		},
