...
func TestBackupCredentialsSecretLifecycle(t *testing.T) {
	...
	secret, err := CreateBackupCredentialsSecret(
		...
		"sessions",
		"backup-test",
		map[string][]byte{BackupAccessKeyDataKey: []byte("access"), BackupSecretKeyDataKey: []byte("secret")},
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if string(loaded.Data[BackupAccessKeyDataKey]) != "access" || string(loaded.Data[BackupSecretKeyDataKey]) != "secret" {
		t.Fatalf("credentials data = %#v", loaded.Data)
	}
	if _, err := GetBackupCredentialsSecret(context.Background(), client, ref, "other-session"); err == nil {
		t.Fatal("expected ownership mismatch")
	}
	if err := DeleteBackupCredentialsSecret(context.Background(), client, ref, "backup-test"); err != nil {
		t.Fatal(err)
	}
}
...
func TestBackupCredentialsSecretRejectsReplacementAndWrongName(t *testing.T) {
	...
	if _, err := GetBackupCredentialsSecret(context.Background(), client, ref, "backup-test"); domain.CategoryOf(err) != domain.ErrorConflict {
		t.Fatalf("replacement category=%s error=%v", domain.CategoryOf(err), err)
	}
	if err := ValidateBackupCredentialsSecretCleanup(context.Background(), client, ref, "backup-test"); domain.CategoryOf(err) != domain.ErrorConflict {
		t.Fatalf("cleanup validation category=%s error=%v", domain.CategoryOf(err), err)
	}
	if err := DeleteBackupCredentialsSecret(context.Background(), client, ref, "backup-test"); domain.CategoryOf(err) != domain.ErrorConflict {
...
func TestBackupCredentialsSecretMissingIsActionable(t *testing.T) {
	...
	if _, err := GetBackupCredentialsSecret(context.Background(), client, ref, "backup-test"); domain.CategoryOf(err) != domain.ErrorPrecondition {
		t.Fatalf("missing Secret category=%q error=%v", domain.CategoryOf(err), err)
	}
	if err := ValidateBackupCredentialsSecretCleanup(context.Background(), client, ref, "backup-test"); err != nil {
