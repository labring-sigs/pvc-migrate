...
func TestReserveVolumeProvisionsOnTargetAndRetainsBothPVs(t *testing.T) {
	...
	client := fake.NewClientset(
		&corev1.PersistentVolumeClaim{
			...
			Spec:       corev1.PersistentVolumeClaimSpec{VolumeName: "pv-source", StorageClassName: &storageClass, VolumeMode: &mode},
	...
	client.PrependReactor("create", "pods", func(action clienttesting.Action) (bool, runtime.Object, error) {
		...
		if err := client.Tracker().Update(corev1.SchemeGroupVersion.WithResource("persistentvolumeclaims"), pvc, "system"); err != nil {
			return true, nil, err
		}
		if err := client.Tracker().Create(corev1.SchemeGroupVersion.WithResource("persistentvolumes"), &corev1.PersistentVolume{
			...
			Spec: corev1.PersistentVolumeSpec{
				Capacity:                      corev1.ResourceList{corev1.ResourceStorage: resource.MustParse("1Gi")},
				PersistentVolumeReclaimPolicy: corev1.PersistentVolumeReclaimDelete,
				ClaimRef:                      &corev1.ObjectReference{Namespace: "system", Name: "data-migrated", UID: pvc.UID},
			},
			Status: corev1.PersistentVolumeStatus{Phase: corev1.VolumeBound},
		}, ""); err != nil {
			return true, nil, err
	...
	session := domain.NewSession("session", domain.NewSessionSpec(domain.OperationReserve, domain.SessionCommon{
		...
		Volumes: []domain.VolumeSpec{{
			SourcePVC:      domain.ObjectReference{Namespace: "app", Name: "data", UID: sourcePVCUID},
	...
	}, domain.WorkloadSpec{Adapter: domain.WorkloadNone}, false, domain.SessionWorkflowOptions{TargetNode: "node-b"}), time.Now())
