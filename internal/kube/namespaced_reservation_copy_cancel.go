...
// NamespacedCancelCRDReservationCopyHandoff returns cleanup responsibility to the original
// Reservation. The caller holds the same Lease used by adoption and execution.
func NamespacedCancelCRDReservationCopyHandoff(
	ctx context.Context,
...
func NamespacedValidatePendingReservationCopy(
	source *v1alpha1.Reservation,
	target *v1alpha1.Copy,
) error {
	if source == nil || target == nil || source.UID == "" || source.Name == "" ||
		source.Namespace == "" || source.Name != target.Name || source.Namespace != target.Namespace {
		return workflowStoreConflict(
			"cancel handoff",
			"reservation and copy must share a namespaced identity",
		)
	}
