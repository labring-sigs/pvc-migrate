...
func TestConfigMapSessionStoreRoundTripAndConflict(t *testing.T) {
	...
	updatedMetadata, err := client.CoreV1().
		ConfigMaps("system").
		Get(ctx, SessionConfigMapName("alpha"), metav1.GetOptions{})
	if err != nil {
		t.Fatal(err)
	...
	if !containsString(updatedMetadata.Finalizers, SessionFinalizer) ||
		...
		t.Fatalf(
			"session metadata was not preserved: finalizers=%v annotations=%v",
			updatedMetadata.Finalizers,
	...
	client.PrependReactor(
		...
		func(action clienttesting.Action) (bool, runtime.Object, error) {
			return true, nil, apierrors.NewConflict(
				...
				errors.New("resourceVersion is stale"),
			)
		},
	)
...
func TestConfigMapSessionStoreDeleteUsesUIDPrecondition(t *testing.T) {
	...
	if preconditions == nil || preconditions.UID == nil ||
		*preconditions.UID != "session-configmap-uid" ||
...
func TestConfigMapSessionStoreRejectsOwnershipMetadataMismatch(t *testing.T) {
	...
	if _, err := client.CoreV1().
		ConfigMaps("system").
		Get(ctx, SessionConfigMapName("alpha"), metav1.GetOptions{}); err != nil {
