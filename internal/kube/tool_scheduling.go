...
func ToolComponentNodeHelmValues(component string, node *corev1.Node) ([]string, error) {
	if !validToolProbeComponent(component) || component == ToolComponentShell {
		return nil, domain.NewError(domain.ErrorValidation, "tool scheduling", fmt.Sprintf("unsupported tool component %q", component))
	}
	if node == nil || node.Name == "" {
		return nil, domain.NewError(domain.ErrorKubernetes, "tool scheduling", "node is empty")
	}
	values := []string{component + ".nodeName=" + node.Name}
	return append(values, ToolComponentTolerationHelmValues(component, node)...), nil
}
