		...
		func(action clienttesting.Action) (bool, runtime.Object, error) {
			...
			if updates == 1 {
				return true, nil, apierrors.NewConflict(
					...
					ref.Name,
					errors.New("injected concurrent update"),
				)
			}
...
func TestReleasePVCIsOwnershipSafeAndIdempotent(t *testing.T) {
	...
	if _, exists := pvc.Annotations[SessionKey]; exists ||
		pvc.Annotations["example.com/keep"] != "value" {
...
func TestFinalizePVCRestoresOriginalMetadataAndPreservesBindingAnnotations(t *testing.T) {
	...
	client := fake.NewClientset(&corev1.PersistentVolumeClaim{ObjectMeta: metav1.ObjectMeta{
		...
		Annotations: map[string]string{
			SessionKey:                        "session",
			RollbackPVAnnotation:              "old-pv",
			SourcePVAnnotation:                "source-pv",
			SourcePVCUIDAnnotation:            "source-pvc",
	...
	if pvc.Labels[ManagedByLabel] != "database-operator" || pvc.Labels[SessionKey] != "" ||
		pvc.Labels["external.example/keep"] != "value" ||
		pvc.Labels["original.example/label"] != "value" {
		t.Fatalf("labels=%v", pvc.Labels)
	}

	if pvc.Annotations[SessionKey] != "" || pvc.Annotations[RollbackPVAnnotation] != "" ||
		pvc.Annotations[SourcePVAnnotation] != "" ||
		pvc.Annotations[SourcePVCUIDAnnotation] != "" ||
		pvc.Annotations["pv.kubernetes.io/bind-completed"] != "yes" ||
		pvc.Annotations["original.example/annotation"] != "value" {
