...
func configMapHandoffFixture(
	t *testing.T,
) (*fake.Clientset, *v1alpha1.ClusterReservation, *v1alpha1.ClusterCopy) {
	t.Helper()
	...
	source := &v1alpha1.ClusterReservation{
		...
		ObjectMeta: metav1.ObjectMeta{Name: "workflow", UID: "storage-uid", ResourceVersion: "1"},
			...
			Volumes: []v1alpha1.ClusterReservationVolumeStatus{
				{
					...
					DestinationPV: &v1alpha1.ObjectReference{Name: "reserved-pv", UID: "pv"},
				},
			},
		},
	}
	...
	destination := &v1alpha1.ClusterCopy{
		...
		Status: v1alpha1.ClusterCopyStatus{
			...
			Volumes: []v1alpha1.ClusterCopyVolumeStatus{
				{
					ClusterVolumeReservationStatus: *source.Status.Volumes[0].ClusterVolumeReservationStatus.DeepCopy(),
				},
			},
		},
	}
...
func TestConfigMapReservationHandoffCommitsCompleteCopyAtomically(t *testing.T) {
	client, source, destination := configMapHandoffFixture(t)

	before := source.DeepCopy()
	if err := HandoffConfigMapReservationToCopy(
		t.Context(),
		client,
		"sessions",
		source,
	...
	loaded, err := store.Load(t.Context(), crclient.ObjectKey{Name: source.Name})
	if err != nil {
		t.Fatal(err)
	}
	...
	// lists select records by workflow-kind, so a ClusterCopy record that
	// still carries the reservation label vanishes from every list.
	...
	if record.Labels[WorkflowKindLabel] != "ClusterCopy" {
		t.Fatalf(
			"record workflow-kind label = %q, want ClusterCopy",
			record.Labels[WorkflowKindLabel],
		)
	}
...
func TestConfigMapReservationHandoffFailurePreservesBothInputs(t *testing.T) {
	for _, mode := range []string{"save", "fence", "stale"} {
		t.Run(mode, func(t *testing.T) {
			...
			if _, err := store.Load(
				t.Context(),
				crclient.ObjectKey{Name: source.Name},
			); err != nil {
