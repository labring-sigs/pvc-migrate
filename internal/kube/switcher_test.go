...
func TestSwitcherWaitLogsBeforePolling(t *testing.T) {
	...
	if !strings.Contains(logs.String(), "waiting for Kubernetes resource") ||
		!strings.Contains(logs.String(), "PVC binding") {
...
func switcherFixture(
	t *testing.T,
) (*Switcher, *domain.Session, *domain.VolumeSpec, *domain.VolumeStatus) {
	t.Helper()
	...
	destinationPV := &corev1.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				OriginalPolicyAnnotation: string(corev1.PersistentVolumeReclaimDelete),
			},
		},
		Spec: corev1.PersistentVolumeSpec{
			Capacity: corev1.ResourceList{
		...
		func(action clienttesting.Action) (bool, runtime.Object, error) {
			...
			case "app/data":
				pvcObject, err := client.Tracker().
					Get(corev1.SchemeGroupVersion.WithResource("persistentvolumeclaims"), "app", "data")
				if err != nil {
					return true, nil, err
				}
			...
			if pvName != "" {
				pvObject, err := client.Tracker().
					Get(corev1.SchemeGroupVersion.WithResource("persistentvolumes"), "", pvName)
				if err != nil {
					return true, nil, err
				}

				pv := testutil.MustType[*corev1.PersistentVolume](t, pvObject)
	...
	client.PrependReactor(
		...
		"persistentvolumeclaims",
		func(action clienttesting.Action) (bool, runtime.Object, error) {
			pvc := testutil.MustActionObject[*corev1.PersistentVolumeClaim](t, action)
			...
			pvObject, err := client.Tracker().
				Get(corev1.SchemeGroupVersion.WithResource("persistentvolumes"), "", pvc.Spec.VolumeName)
			if err != nil {
				return true, nil, err
			}
...
func TestActivateVolumeRecoversAfterTemporaryPVCDeletion(t *testing.T) {
	...
	err := switcher.ActivateVolume(
		ctx,
		session,
		volume,
		status,
		func() error { return progressFailure },
	)
	...
	updates := 0
	if err := switcher.ActivateVolume(
		ctx,
		session,
		volume,
		status,
	...
	if active.Spec.VolumeName != "pv-destination" || active.Labels["app"] != "database" ||
		len(active.OwnerReferences) != 1 {
	...
	if active.Annotations["example.com/retained"] != "value" ||
		active.Annotations["pv.kubernetes.io/bind-completed"] != "" ||
		active.Annotations["volume.kubernetes.io/selected-node"] != "" ||
		active.Annotations["volume.kubernetes.io/storage-provisioner"] != "" {
		t.Fatalf("active PVC annotations: %#v", active.Annotations)
	}

	destination, _ := switcher.client.CoreV1().
		PersistentVolumes().
		Get(ctx, "pv-destination", metav1.GetOptions{})
	if destination.Spec.ClaimRef == nil || destination.Spec.ClaimRef.UID != active.UID ||
		destination.Labels[ResourceRoleLabel] != "active" {
...
func TestRollbackVolumeRestoresOriginalPV(t *testing.T) {
	...
	if active.Spec.VolumeName != "pv-source" ||
		active.Annotations[RollbackPVAnnotation] != "pv-destination" ||
		status.Activation.RolledBackAt == nil {
	...
	if rollbackCapacity.Cmp(resource.MustParse("1Gi")) != 0 {
		t.Fatalf(
			"rollback PVC did not restore source capacity: %#v",
			active.Spec.Resources.Requests,
		)
	}

	source, _ := switcher.client.CoreV1().
		PersistentVolumes().
		Get(ctx, "pv-source", metav1.GetOptions{})

	destination, _ := switcher.client.CoreV1().
		PersistentVolumes().
		Get(ctx, "pv-destination", metav1.GetOptions{})
	if source.Labels[ResourceRoleLabel] != "active" ||
		destination.Labels[ResourceRoleLabel] != "rollback" {
		t.Fatalf(
			"PV roles: source=%q destination=%q",
			source.Labels[ResourceRoleLabel],
			destination.Labels[ResourceRoleLabel],
		)
	}
}
...
func TestRollbackVolumeRetainsDestinationBeforeDeletingActivePVC(t *testing.T) {
	...
	destination, err := switcher.client.CoreV1().
		PersistentVolumes().
	...
	if _, err := switcher.client.CoreV1().
		PersistentVolumes().
		Update(ctx, destination, metav1.UpdateOptions{}); err != nil {
		t.Fatal(err)
	}
	...
	client.PrependReactor(
		...
		"persistentvolumes",
		func(action clienttesting.Action) (bool, runtime.Object, error) {
			updateAction := testutil.MustType[clienttesting.UpdateAction](t, action)

			pv := testutil.MustType[*corev1.PersistentVolume](t, updateAction.GetObject())
			if pv.Name == volume.DestinationPV.Name &&
	...
	client.PrependReactor(
		...
		func(action clienttesting.Action) (bool, runtime.Object, error) {
			deleteAction := testutil.MustType[clienttesting.DeleteAction](t, action)
			if deleteAction.GetNamespace() != volume.SourcePVC.Namespace ||
				deleteAction.GetName() != volume.SourcePVC.Name {
				return false, nil, nil
			}

			if !retainedBeforeDelete {
				return true, nil, errors.New(
					"destination PV was deleted before reclaim policy was restored",
				)
			}
...
func TestActivateVolumeRejectsReusedSourcePVCName(t *testing.T) {
	...
	switcher, session, volume, status := switcherFixture(t)
	pvc, _ := switcher.client.CoreV1().
		PersistentVolumeClaims("app").
		Get(ctx, "data", metav1.GetOptions{})
	pvc.UID = types.UID("reused-name-uid")
	_, _ = switcher.client.CoreV1().
		PersistentVolumeClaims("app").
