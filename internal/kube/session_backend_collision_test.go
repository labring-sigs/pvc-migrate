	...
	for _, namespace := range []string{"system", "source"} {
		t.Run(namespace, func(t *testing.T) {
			...
			if domain.CategoryOf(err) != domain.ErrorConflict ||
				!strings.Contains(err.Error(), "ConfigMap session") {
...
func TestCRDSessionCollisionChecksFailClosedForExecution(t *testing.T) {
	...
	if err := store.Create(t.Context(), session); err != nil {
		t.Fatalf(
			"tenant submission should defer unreadable collision checks to controller: %v",
			err,
		)
	}

	if err := store.CheckWorkflowNameCollision(
		t.Context(),
		session,
	); domain.CategoryOf(
		err,
...
func TestCRDSessionRechecksConfigMapAfterAcquiringLease(t *testing.T) {
	...
	client.PrependReactor(
		...
		func(action clienttesting.Action) (bool, runtime.Object, error) {
			createAction := testutil.MustType[clienttesting.CreateAction](t, action)
			lease := testutil.MustType[*coordinationv1.Lease](t, createAction.GetObject())
	...
	lease, err := client.CoordinationV1().
		Leases("system").
		Get(t.Context(), SessionLockName("alpha"), metav1.GetOptions{})
