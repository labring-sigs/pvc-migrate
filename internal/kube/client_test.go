...
func TestEnsureNamespaceCreateExistingAndDryRun(t *testing.T) {
	...
	dryRunClient.PrependReactor("create", "namespaces", func(action clienttesting.Action) (bool, runtime.Object, error) {
	...
	if err := EnsureNamespace(ctx, client, "", "session", false); domain.CategoryOf(err) != domain.ErrorValidation {
...
`)
	...
	if clients.Kubernetes == nil || clients.Dynamic == nil || clients.Discovery == nil || clients.RESTConfig == nil {
...
func TestWaitForReadyErrorAndTimeout(t *testing.T) {
	calls := 0
	if err := WaitFor(context.Background(), time.Millisecond, "ready", func(context.Context) (bool, error) {
		calls++
		return calls == 2, nil
	}); err != nil {
		t.Fatal(err)
	...
	if err := WaitFor(context.Background(), time.Millisecond, "error", func(context.Context) (bool, error) {
...
func TestParseGroupVersionResource(t *testing.T) {
	...
	if _, err := ParseGroupVersionResource("too/many/segments", "pods"); domain.CategoryOf(err) != domain.ErrorValidation {
