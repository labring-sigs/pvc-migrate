...
func (s *Service) ValidateOfflineMigrationResume(
	...
	if err := requireWorkflowSession(session, domain.SessionTypeMigrate, "resume migrate"); err != nil {
		return err
	}
...
func (s *Service) ValidateOfflineMigrationAbort(ctx context.Context, session *domain.Session) error {
	if err := requireWorkflowSession(session, domain.SessionTypeMigrate, "abort migrate"); err != nil {
		return err
	}
...
func (s *Service) ValidateOfflineMigrationRollback(ctx context.Context, session *domain.Session) error {
	if err := requireWorkflowSession(session, domain.SessionTypeMigrate, "rollback migrate"); err != nil {
		return err
	}
...
func (s *Service) ValidateOfflineMigrationCleanup(
	...
	if err := requireWorkflowSession(session, domain.SessionTypeMigrate, "cleanup migrate"); err != nil {
		return err
	}
