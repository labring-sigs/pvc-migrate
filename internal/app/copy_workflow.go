...
func (s *Service) ValidateCopyCleanup(ctx context.Context, session *domain.Session, options CleanupOptions) error {
...
func (s *Service) CleanupCopy(ctx context.Context, session *domain.Session, options CleanupOptions) error {
	if err := requireWorkflowSession(session, domain.SessionTypeCopy, "cleanup copy"); err != nil {
		return err
	}
