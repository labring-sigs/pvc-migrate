...
func TestNamespacedMigrationActivationRecoversResourceChangeBeforeCheckpoint(t *testing.T) {
...
func TestNamespacedMigrationActivationRejectsMissingFinalSyncBeforeStorageAccess(t *testing.T) {
	executor, object, store, _ := namespacedMigrationFixture(t)
	if err := executor.Reserve(t.Context(), object); err != nil {
		t.Fatal(err)
	}
