...
func TestNamespacedPodMigrationActivationRecoversResourceChangeBeforeCheckpoint(t *testing.T) {
...
func TestNamespacedPodMigrationActivationRejectsMissingFinalSyncBeforeStorageAccess(t *testing.T) {
	executor, object, store, _ := namespacedPodMigrationFixture(t)
	if err := executor.Reserve(t.Context(), object); err != nil {
		t.Fatal(err)
	}
