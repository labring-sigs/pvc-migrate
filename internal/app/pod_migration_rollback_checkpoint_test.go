...
func TestPodMigrationRollbackCheckpointsCurrentPodBeforePause(t *testing.T) {
	...
	controller := &rollbackCheckpointController{
		current: []v1alpha1.ObjectReference{
			{Name: "workload", Namespace: "source", UID: "current", ResourceVersion: "17"},
		},
	}
...
func TestPodMigrationRollbackResumeFailureDoesNotPauseRestoredWorkloadAgain(t *testing.T) {
	executor, object, store, _ := podMigrationExecutorFixture(t)
	executor.workloads = &fakeController{}
	executor.transfer.copier = &concreteCopyEngine{}
	executor.transfer.config.Retries = 1
...
func TestPodMigrationRollbackRejectsUnrelatedConsumerBeforePause(t *testing.T) {
	executor, object, store, _ := podMigrationExecutorFixture(t)
	executor.workloads = &fakeController{}
	executor.transfer.copier = &concreteCopyEngine{}
	executor.transfer.config.Retries = 1
	...
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{Name: "foreign", Namespace: "source", UID: "foreign"},
		Spec: corev1.PodSpec{
