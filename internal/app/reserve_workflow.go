...
func (s *Service) ValidateReserveResume(ctx context.Context, session *domain.Session) error {
	if err := requireWorkflowSession(session, domain.SessionTypeReserve, "resume reserve"); err != nil {
		return err
	}
...
func (s *Service) ValidateReserveAbort(ctx context.Context, session *domain.Session) error {
	if err := requireWorkflowSession(session, domain.SessionTypeReserve, "abort reserve"); err != nil {
		return err
	}
...
func (s *Service) ValidateReserveCleanup(ctx context.Context, session *domain.Session, options CleanupOptions) error {
	if err := requireWorkflowSession(session, domain.SessionTypeReserve, "cleanup reserve"); err != nil {
		return err
	}
	return s.validateCleanup(ctx, session, options)
}

func (s *Service) CleanupReserve(ctx context.Context, session *domain.Session, options CleanupOptions) error {
	if err := requireWorkflowSession(session, domain.SessionTypeReserve, "cleanup reserve"); err != nil {
		return err
	}
