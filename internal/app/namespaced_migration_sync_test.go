...
func TestNamespacedMigrationFinalSyncResumesOnlyIncompleteVolumes(t *testing.T) {
...
func TestNamespacedMigrationFinalSyncRestartRollsBackOnCheckpointFailure(t *testing.T) {
	executor, object, store, _ := namespacedMigrationFixture(t)
	engine := &concreteCopyEngine{}
	executor.transfer.copier = engine
	executor.transfer.config.Retries = 1
...
func TestNamespacedMigrationFinalSyncRejectsIncompleteReservation(t *testing.T) {
	executor, object, store, _ := namespacedMigrationFixture(t)
	object.Status.Phase = domain.PhaseFinalSyncing

	object.Status.Volumes = []v1alpha1.MigrationVolumeStatus{{}}
