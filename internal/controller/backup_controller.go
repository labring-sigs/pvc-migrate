...
type BackupPlanner func(context.Context, *v1alpha1.Backup, string) error

type BackupReconciler struct {
	store          *kube.CRDWorkflowStore[*v1alpha1.Backup]
...
func (r *BackupReconciler) executor(namespace string) *backup.BackupExecutor {
	return backup.NewBackupExecutor(r.client, r.store, r.locker, namespace, r.config)
}
...
func (r *BackupReconciler) plan(ctx context.Context, object *v1alpha1.Backup) error {
	if r.planner == nil {
		return errors.New("backup planner is required")
	}
	...
	return withPlanningLease(
		...
		func(ctx context.Context) error {
			...
			if cause == nil && object.Status.Plan == nil {
				cause = errors.New("backup planner returned no execution plan")
			}
...
func (r *WorkflowReconciler) configureBackupController(
	client crclient.Client,
	options ManagerOptions,
	locker kube.SessionLocker,
) error {
	if !r.supportsKind(domain.ControllerKindBackup) {
		return nil
	}

	if options.BackupPlanner == nil {
		return errors.New("backup planner is required")
	}
	...
	r.backup = &BackupReconciler{
		store: store, client: options.KubernetesClient, planner: options.BackupPlanner,
