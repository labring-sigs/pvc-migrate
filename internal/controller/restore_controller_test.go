...
func TestRestoreControllerOwnsPlanningResumeAndDeletion(t *testing.T) {
	object := &v1alpha1.Restore{
		ObjectMeta: metav1.ObjectMeta{Name: "restore", Namespace: "tenant", UID: "workflow"},
	...
	client := crfake.NewClientBuilder().
		...
		WithObjects(object).
		Build()
	r := NewWorkflowReconciler().WithSupportedKinds([]domain.ControllerKind{domain.ControllerKindRestore}).
	...
	options := ManagerOptions{
		...
		RestorePlanner: func(_ context.Context, request *v1alpha1.Restore, image string) error {
	...
	r.restore.checkCollision = func(_ context.Context, namespace, name string) error {
		if name != object.Name || namespace != object.Namespace {
			t.Fatal("collision check escaped operation identity")
		}
		return nil
	}
	entry := &kindWorkflowReconciler{parent: r, kind: domain.ControllerKindRestore}
	...
	executor := r.restore.executor(object.Namespace)
